 ...
 * @returns {*}
 */
function parseValue(ctx, minIndent) {
    ...
    // Plain scalar
    return parsePlainScalar(ctx, minIndent);
}
...
function parseFlowValue(ctx) {
    ...
    return parseFlowPlainScalar(ctx);
}
 ...
 * @returns {*}
 */
function parseFlowPlainScalar(ctx) {
    let value = "";
    while (ctx.pos < ctx.len) {
        const ch = peekCode(ctx);
        if (
        ...
        if (ch === CH_DASH) {
            ...
            if (
                ...
                if (c === CH_LF || c === CH_CR || c === CH_HASH) {
                    ...
                    result.push(parseValue(ctx, lineIndent + 2));
                }
