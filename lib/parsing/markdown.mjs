 ...
 * | "link" | "image" | "form-field"} MarkdownNodeType
 */
...
/**
 * @typedef {{ node: MarkdownNode; end: number }} MarkdownImageReference
 */
 ...
 * @param {string} source
 * @param {number} start
 * @returns {MarkdownImageReference | null}
 */
function parseImageReference(source, start) {
    ...
    for (; position < source.length; position++) {
        ...
        if (source.charCodeAt(position) === CHAR_BACKSLASH) {
            position++;
            continue;
        }
        if (character === "[") depth++;
    ...
    if (depth !== 0 || source[position + 1] !== "(") return null;
    const label = source.slice(start + 2, position);
    position += 2;
    ...
    if (source[position] === "<") {
        ...
        while (position < source.length && source[position] !== ">") {
            if (source[position] === "\n" || source[position] === "\r")
                return null;
            if (source.charCodeAt(position) === CHAR_BACKSLASH) position++;
            position++;
        }
    ...
    if (position > beforeWhitespace && (quote === '"' || quote === "'")) {
        const begin = ++position;
        ...
        if (source[position] !== quote) return null;
        title = source.slice(begin, position++).replace(/\\([\\"'])/g, "$1");
    ...
    return {
        node: {
            type: "image",
            content: label.replace(/\\([\\\[\]!])/g, "$1"),
            children: [],
            attrs: {
                src: destination.replace(/\\([\\()<> ])/g, "$1"),
...
function parseInlineContent(content) {
    ...
    while (pos < len) {
        ...
        if (ch === CHAR_NEWLINE || ch === CHAR_CR) {
            nodes.push({ type: "line-break", content: "", children: [] });
            pos +=
                ch === CHAR_CR && content.charCodeAt(pos + 1) === CHAR_NEWLINE
                    ? 2
                    : 1;
            continue;
        }
        ...
        if (
            ch === CHAR_EXCLAIM &&
            content.charCodeAt(pos + 1) === CHAR_OPEN_BRACKET
        ) {
            const image = parseImageReference(content, pos);
            if (image) {
        ...
        while (textEnd < len) {
            ...
            if (
                ...
                c === CHAR_BACKSLASH ||
                (c === CHAR_EXCLAIM &&
                    content.charCodeAt(textEnd + 1) === CHAR_OPEN_BRACKET) ||
                c === CHAR_STAR ||
                c === CHAR_BACKTICK ||
                c === CHAR_UNDERSCORE ||
                c === CHAR_OPEN_BRACKET
...
/**
 * Skip one line-ending sequence without consuming a blank line.
 * @param {string} markdown
 * @param {number} pos
 * @param {number} len
 * @returns {number}
 */
function skipNewline(markdown, pos, len) {
    if (pos >= len) return pos;
    const char = markdown.charCodeAt(pos);
    if (char === CHAR_CR) {
        return pos + (markdown.charCodeAt(pos + 1) === CHAR_NEWLINE ? 2 : 1);
    }
    return char === CHAR_NEWLINE ? pos + 1 : pos;
}
...
function parseDirectiveBlock(
    ...
    while (pos < end) {
        ...
        const fence = lineText.match(/^(`{3,}|~{3,})/);
        if (codeFence) {
            if (
                fence &&
            ...
            codeFence = fence[1];
        } else if (/^:::[ \t]*[A-Za-z]/.test(lineText)) {
            directiveDepth++;
        } else if (lineText === ":::" && directiveDepth > 0) {
            directiveDepth--;
        } else if (lineText === ":::") {
            contentEnd = pos;
    ...
    const inner =
        innerText.length > 0
            ? parseMarkdownDoc(innerText, options)
            : { nodes: [], index: {} };
...
function parseMarkdownDoc(markdown, options) {
    ...
    while (pos < len) {
        ...
        let backslashStart = lineEnd;
        while (
            backslashStart > firstNonSpace &&
            markdown.charCodeAt(backslashStart - 1) === CHAR_BACKSLASH
        )
            backslashStart--;
        const backslashBreak = (lineEnd - backslashStart) % 2 === 1;
        const hardBreakThisLine =
            lineEnd < len && (lineEnd - lastNonSpace >= 2 || backslashBreak);
        const contentEnd = hardBreakThisLine
            ? backslashBreak
                ? lineEnd - 1
                : lastNonSpace
            : lineEnd;
        ...
        if (
            /^\*\*[^*\r\n]+:\*\*(?:[ \t]|$)/.test(
                markdown.slice(firstNonSpace, lastNonSpace)
            )
        ) {
            lastParagraph = undefined;
            lastListItem = undefined;
        ...
        if (indentSpaces <= 3) {
            ...
            if (isPipeTableHeaderLine(rawLine)) {
                const sepStart = skipNewline(markdown, lineEnd, len);
                if (sepStart < len) {
        ...
        if (firstChar === CHAR_DASH) {
            ...
            prevLineHardBreak = hardBreakThisLine;
            // Nested lists retain their owning paragraph for indented continuation.
            if (indentSpaces <= lastParagraphIndentSpaces)
                lastParagraph = undefined;
            continue;
        }
        ...
        // instead of creating a new paragraph node. A line that opens a
        // numbered or lettered item ("2. ", "(b) ", "a) ") is the next item,
        // not a wrapped continuation of the previous one; an explicit hard
        // break still joins it.
        if (
            (prevLineHardBreak ||
                (prevLineSoftJoin &&
                    !isLegalSubItemStart(
                        markdown.slice(firstNonSpace, contentEnd)
                    ))) &&
            !lastListItem &&
            lastParagraph &&
            ...
            const continuationContent = markdown.slice(
                firstNonSpace,
                hardBreakThisLine ? contentEnd : lastNonSpace
            );
...
function reflowMarkdown(text, maxWidth, options) {
    ...
    while (i < len) {
        ...
        const directive = /^:::[ \t]*([A-Za-z][^ \t]*)/.exec(trimmed);
        if (directive) {
            const { endPos } = parseDirectiveBlock(
                text,
                lineOffsets[i] + line.length,
                text.length,
                directive[1],
                ""
            );
            ...
            } while (i < len && lineOffsets[i] < endPos);
            continue;
        }

        if (
            line.includes("![") &&
            parseInlineContent(line).some((node) => node.type === "image")
        ) {
            outputLines.push(line);
            i++;
            continue;
        ...
        if (isListItem(line)) {
            ...
            while (i < len) {
                ...
                if (
                    nextLine.includes("![") &&
                    parseInlineContent(nextLine).some(
