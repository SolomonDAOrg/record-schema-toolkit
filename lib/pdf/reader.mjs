...
export class PdfEditor {
    ...
    save() {
        ...
        const infoRaw = getRaw(this.trailer, "Info");
        if (infoRaw) trailer += "  /Info " + infoRaw + "\n";
    ...
    /** Toolkit form fields are flat, indirect widget dictionaries. */
    getFormFields() {
        const catalog = this.getDict(this.rootId);
        const raw = getRaw(catalog.entries, "AcroForm");
        if (!raw) return [];
        const id = refId(raw);
        if (id === null) throw new Error("Signing requires an indirect AcroForm dictionary.");
        const fields = getRaw(this.getDict(id).entries, "Fields") || "[]";
        if (!/^\[\s*(?:\d+\s+0\s+R\s*)*\]$/.test(fields.trim()))
            throw new Error("Signing requires a flat array of indirect fields.");
        return [...fields.matchAll(/(\d+)\s+0\s+R/g)].map(match => {
            const fieldId = Number(match[1]);
            const entries = this.getDict(fieldId).entries;
            if (getRaw(entries, "Kids")) throw new Error("Hierarchical fields are outside this signing profile.");
            return {id: fieldId, entries};
        });
    }

    /** @param {string} key */
    getInfoRaw(key) {
        const raw = getRaw(this.trailer, "Info");
        const id = raw ? refId(raw) : null;
        return id === null ? undefined : getRaw(this.getDict(id).entries, key);
    }

    /** Read the Unicode PDF Info strings emitted by this toolkit. @param {string} key */
    getInfoText(key) {
        const raw = this.getInfoRaw(key);
        if (raw === undefined) return undefined;
        if (/^<FEFF(?:[0-9A-Fa-f]{4})*>$/.test(raw))
            return Buffer.from(raw.slice(5, -1), "hex").swap16().toString("utf16le");
        if (/^\([^\\()]*\)$/.test(raw)) return raw.slice(1, -1);
        throw new Error("Unsupported Info text encoding for " + key);
    }

    /** Expose signature values for independent CMS verification. */
    getSignatureFields() {
        return this.getFormFields().filter(field => getRaw(field.entries, "FT") === "/Sig").map(field => {
            const name = getRaw(field.entries, "T") || "";
            if (!/^\([A-Za-z][A-Za-z0-9_.]*\)$/.test(name))
                throw new Error("Signing profile requires explicit ASCII field identifiers.");
            const value = getRaw(field.entries, "V");
            const valueId = value && value !== "null" ? refId(value) : null;
            if (value && value !== "null" && valueId === null) throw new Error("Unsupported signature value.");
            const entries = valueId === null ? null : this.getDict(valueId).entries;
            return {
                fieldName: name.slice(1, -1), fieldId: field.id, valueId,
                byteRange: entries ? getRaw(entries, "ByteRange") : undefined,
                contents: entries ? getRaw(entries, "Contents") : undefined,
                subFilter: entries ? getRaw(entries, "SubFilter") : undefined
            };
        });
    }
     ...
     * @param {{ fieldName?: string, existingField?: boolean, name?: string, reason?: string, location?: string, signingTime?: Date, reservedBytes?: number }} [options]
     * @returns {this}
     */
    addSignatureField(options) {
        const opts = options || {};
        if (this.decryptor) throw new Error("Encrypted PDFs cannot be signed by this profile.");
        if (getRaw(this.getDict(this.rootId).entries, "Perms")) throw new Error("Certified PDFs require a permission-aware signing provider.");
        if (this.sigInfo) throw new Error("Only one signature may be staged per incremental update.");
        if (opts.reservedBytes !== undefined && (!Number.isInteger(opts.reservedBytes) || opts.reservedBytes < 1024 || opts.reservedBytes > 1048576)) throw new Error("Invalid signature reservation size.");
        const reservedHex = opts.reservedBytes ? opts.reservedBytes * 2 : 16384;

        const sigValueId = ++this.maxId;
        const sigFieldId = opts.existingField ? null : ++this.maxId;
        ...
        if (opts.existingField) {
            if (!opts.fieldName) throw new Error("An existing signature field name is required.");
            const matches = this.getFormFields().filter(field => getRaw(field.entries, "T") === "(" + escapePdfString(opts.fieldName) + ")");
            if (matches.length !== 1) throw new Error("Expected exactly one existing signing field: " + opts.fieldName);
            const field = matches[0];
            if (getRaw(field.entries, "FT") !== "/Sig") throw new Error("Selected field is not a signature field.");
            const value = getRaw(field.entries, "V");
            if (value && value !== "null") throw new Error("Selected signing field is already signed.");
            if (getRaw(field.entries, "Lock")) throw new Error("Field locking requires a permission-aware signing provider.");
            const entries = field.entries.map(entry => ({...entry}));
            setEntry(entries, "V", formId_ref(sigValueId));
            const rectangle = (getRaw(entries, "Rect") || "").match(/^\[\s*(-?[\d.]+)\s+(-?[\d.]+)\s+(-?[\d.]+)\s+(-?[\d.]+)\s*\]$/);
            if (!rectangle) throw new Error("Signing widget has no supported rectangle.");
            const width = Number(rectangle[3]) - Number(rectangle[1]);
            const height = Number(rectangle[4]) - Number(rectangle[2]);
            if (!Number.isFinite(width) || !Number.isFinite(height) || width < 100 || height < 24) throw new Error("Signing widget is too small for a digital signature appearance.");
            const fontId = ++this.maxId;
            const appearanceId = ++this.maxId;
            this.added.push({id: fontId, kind: "dict", text: createType1Font("Helvetica")});
            const lines = ["Digitally signed", opts.signingTime.toISOString()];
            const appearance = encodeUtf8("q\nBT\n/F1 7 Tf\n2 " + (height / 2 + 4) + " Td\n(" + escapePdfString(lines[0]) + ") Tj\n0 -10 Td\n(" + escapePdfString(lines[1]) + ") Tj\nET\nQ\n");
            this.added.push({id: appearanceId, kind: "stream", bytes: appearance, text: "<< /Type /XObject /Subtype /Form /BBox [0 0 " + width + " " + height + "] /Resources << /Font << /F1 " + formId_ref(fontId) + " >> >> /Length " + appearance.length + " >>"});
            setEntry(entries, "AP", "<< /N " + formId_ref(appearanceId) + " >>");

            this.updated.set(field.id, serializeDict(entries));
    ...
    saveSigned(cmsBuilder) {
        ...
        const brIdx = s.indexOf(this.sigInfo.brPlaceholder, this.bytes.length);
        if (brIdx < 0) {
