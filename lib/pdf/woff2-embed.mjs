...
function ru16(d, o) { return (d[o] << 8) | d[o + 1]; }

/** @param {Uint8Array} d @param {number} o @returns {number} */
function ri16(d, o) { const v = ru16(d, o); return v >= 0x8000 ? v - 0x10000 : v; }

/** @param {Uint8Array} d @param {number} o @returns {number} */
function ru32(d, o) { return ((d[o] << 24) | (d[o + 1] << 16) | (d[o + 2] << 8) | d[o + 3]) >>> 0; }

/** @param {Uint8Array} buf @param {number} o @param {number} v */
function wu16(buf, o, v) { buf[o] = (v >> 8) & 0xff; buf[o + 1] = v & 0xff; }

/** @param {Uint8Array} buf @param {number} o @param {number} v */
function wi16(buf, o, v) { if (v < 0) { v += 0x10000; } wu16(buf, o, v); }
...
function concat(arrays) {
    ...
    let p = 0;
    for (let i = 0; i < arrays.length; i++) { out.set(arrays[i], p); p += arrays[i].length; }
    return out;
}
...
function read255UInt16(d, pos) {
    const b0 = d[pos++];
    if (b0 < 253) { return [b0, pos]; }
    if (b0 === 253) {
        const v = (d[pos] << 8) | d[pos + 1]; pos += 2;
        return [v, pos];
    }
    if (b0 === 254) {
        const v = (d[pos] << 8) | d[pos + 1]; pos += 2;
...
function decodeTriplet(xyFlag, gs, pos) {
    ...
    if (xyFlag < 10) {
        ...
        const xRaw = (gs[pos] << 8) | gs[pos + 1]; pos += 2;
        const yRaw = (gs[pos] << 8) | gs[pos + 1]; pos += 2;
...
function reconstructGlyfLoca(tg, numGlyphs, indexFormat) {
    ...
    const nContourStreamSize = ru32(tg, pos); pos += 4;
    const nPointsStreamSize  = ru32(tg, pos); pos += 4;
    const flagStreamSize     = ru32(tg, pos); pos += 4;
    const glyphStreamSize    = ru32(tg, pos); pos += 4;
    const compositeStreamSize = ru32(tg, pos); pos += 4;
    const bboxStreamSize     = ru32(tg, pos); pos += 4;
    const instructionStreamSize = ru32(tg, pos); pos += 4;

    // ---- Slice streams ----
    const nContourStream  = tg.subarray(pos, pos + nContourStreamSize);  pos += nContourStreamSize;
    const nPointsStream   = tg.subarray(pos, pos + nPointsStreamSize);   pos += nPointsStreamSize;
    const flagStream      = tg.subarray(pos, pos + flagStreamSize);      pos += flagStreamSize;
    const glyphStream     = tg.subarray(pos, pos + glyphStreamSize);     pos += glyphStreamSize;
    const compositeStream = tg.subarray(pos, pos + compositeStreamSize); pos += compositeStreamSize;
    ...
    const bboxData        = tg.subarray(pos + bboxBitmapSize, pos + bboxStreamSize); pos += bboxStreamSize;
    ...
    for (let g = 0; g < numGlyphs; g++) {
        ...
        const ncRaw = ru16(nContourStream, ncPos); ncPos += 2;
        ...
        if (nContours < 0) {
            ...
            let more = true;
            while (more) {
                const cFlags = ru16(compositeStream, csPos); csPos += 2;
                csPos += 2; // glyphIndex
                ...
                if (cFlags & 0x0100) { hasInstBit = true; } // WE_HAVE_INSTRUCTIONS
            ...
            if (hasInstBit) {
                instLen = ru16(instructionStream, instPos); instPos += 2;
                instBytes = instructionStream.slice(instPos, instPos + instLen); instPos += instLen;
            }

            // Build composite glyph binary: header(10) + compData + [instLen(2) + instBytes]
            const glyph = new Uint8Array(10 + compData.length + (hasInstBit ? 2 + instLen : 0));
            let gp = 0;
            wi16(glyph, gp, -1); gp += 2;
            wi16(glyph, gp, xMin); gp += 2;
            wi16(glyph, gp, yMin); gp += 2;
            wi16(glyph, gp, xMax); gp += 2;
            wi16(glyph, gp, yMax); gp += 2;
            glyph.set(compData, gp); gp += compData.length;
            if (hasInstBit) {
                wu16(glyph, gp, instLen); gp += 2;
            ...
            for (let p = 0; p < totalPoints; p++) {
                const fb = rawFlags[p];
                onCurve[p] = (fb >> 7) & 1;
                const [dx, dy, newGsPos] = decodeTriplet(fb & 0x7f, glyphStream, gsPos);
                gsPos = newGsPos;
                ax += dx;
                ay += dy;
                xs[p] = ax;
            ...
            if (!hasBbox) {
                ...
                for (let p = 1; p < totalPoints; p++) {
                    if (xs[p] < xMin) { xMin = xs[p]; }
                    if (xs[p] > xMax) { xMax = xs[p]; }
                    if (ys[p] < yMin) { yMin = ys[p]; }
                    if (ys[p] > yMax) { yMax = ys[p]; }
                }
            }
            ...
            const glyphSz = headerSz + endPtsSz + instSz + flagsSz + coordSz * 2;
            const glyph = new Uint8Array(glyphSz);
            let gp = 0;

            wi16(glyph, gp, nContours); gp += 2;
            wi16(glyph, gp, xMin);      gp += 2;
            wi16(glyph, gp, yMin);      gp += 2;
            wi16(glyph, gp, xMax);      gp += 2;
            wi16(glyph, gp, yMax);      gp += 2;
            ...
            for (let c = 0; c < nContours; c++) {
                wu16(glyph, gp, endPts[c]); gp += 2;
            }

            wu16(glyph, gp, instLen); gp += 2;
            glyph.set(instBytes, gp);  gp += instLen;
            ...
            let prevX = 0;
            for (let p = 0; p < totalPoints; p++) {
                wi16(glyph, gp, xs[p] - prevX); gp += 2;
                prevX = xs[p];
            }
            // y deltas
            let prevY = 0;
            for (let p = 0; p < totalPoints; p++) {
                wi16(glyph, gp, ys[p] - prevY); gp += 2;
                prevY = ys[p];
            }
...
function tableChecksum(data) {
    ...
    for (let i = 0; i < words; i++) {
        const b = i * 4;
        const w = (((data[b] || 0) << 24) | ((data[b + 1] || 0) << 16) |
                   ((data[b + 2] || 0) << 8) | (data[b + 3] || 0)) >>> 0;
        sum = (sum + w) >>> 0;
    }
    return sum;
}
...
function buildTtf(tables) {
    ...
    // Offset table
    wu32(ttf, p, 0x00010000); p += 4; // sfVersion = TrueType
    wu16(ttf, p, n);          p += 2;
    wu16(ttf, p, searchRange);   p += 2;
    wu16(ttf, p, entrySelector); p += 2;
    wu16(ttf, p, rangeShift);    p += 2;

    // Table directory
    for (let i = 0; i < n; i++) {
        const tag = tags[i];
        const raw = /** @type {Uint8Array} */ (tables.get(tag));
        const { offset } = /** @type {{padded:Uint8Array;offset:number}} */ (resolved.get(tag));
        for (let j = 0; j < 4; j++) { ttf[p + j] = tag.charCodeAt(j) || 0x20; }
        p += 4;
        wu32(ttf, p, tableChecksum(raw)); p += 4;
        wu32(ttf, p, offset);            p += 4;
        wu32(ttf, p, raw.length);        p += 4;
    }
...
export function parseWoff2(woff2Bytes) {
    ...
    // ---- Validate header ----
    const sig = ru32(d, 0);
    if (sig !== 0x774F4632) { throw new Error("WOFF2: invalid signature (expected 0x774F4632 'wOF2')"); }

    const flavor = ru32(d, 4);
    if (flavor === 0x4F54544F) {
        throw new Error("WOFF2: CFF-outline fonts (flavor 'OTTO') are not supported; only TrueType outline WOFF2 is handled");
    }
    ...
    for (let i = 0; i < numTables; i++) {
        ...
        let tag;
        if (tagIdx === 63) {
            tag = String.fromCharCode(d[pos], d[pos + 1], d[pos + 2], d[pos + 3]);
            pos += 4;
        } else {
            tag = KNOWN_TAGS[tagIdx];
            if (!tag) { throw new Error("WOFF2: unknown tag index " + tagIdx); }
        }

        const [origLength, pos1] = readUBase128(d, pos); pos = pos1;
        ...
        if (hasTransformLength) {
            const [tl, pos2] = readUBase128(d, pos); pos = pos2;
            transformLength = tl;
        }
    ...
    for (let i = 0, len = tableDir.length; i < len; i++) {
        ...
        const data = decompressed.subarray(decomp, decomp + entry.transformLength);
    ...
    const needsGlyfReconstruct = glyfInfo && glyfInfo.entry.transformVersion === 0;

    /** @type {Map<string, Uint8Array>} */
    const ttfTables = new Map();

    for (const [tag, info] of rawTables) {
        if (tag === "glyf" || tag === "loca") { continue; } // handled separately below
    ...
    if (needsGlyfReconstruct && glyfInfo) {
        ...
        if (!maxpData) { throw new Error("WOFF2: missing maxp table (needed for glyf reconstruction)"); }
        if (!headData) { throw new Error("WOFF2: missing head table (needed for glyf reconstruction)"); }

        const numGlyphs  = ru16(maxpData, 4);
        const indexFormat = ru16(headData, 50); // indexToLocFormat: 0=short, 1=long

        const { glyfData, locaData } = reconstructGlyfLoca(glyfInfo.data, numGlyphs, indexFormat);
        ttfTables.set("glyf", glyfData);
        ttfTables.set("loca", locaData);
    } else {
        // Identity transform or no transform: use verbatim
        if (glyfInfo) { ttfTables.set("glyf", glyfInfo.data); }
        const locaInfo = rawTables.get("loca");
        if (locaInfo) { ttfTables.set("loca", locaInfo.data); }
    }
