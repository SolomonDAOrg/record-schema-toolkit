...
export function parseTLV(bytes, pos) {
    if (!Number.isSafeInteger(pos) || pos < 0 || pos + 2 > bytes.length) throw new Error("Truncated DER element.");
    const tlvStart = pos;
    const tag = bytes[pos];
    pos = pos + 1;
    let len = bytes[pos];
    ...
    if (len >= 0x80) {
        const num = len & 0x7f;
        if (num === 0 || num > 4 || pos + num > bytes.length || bytes[pos] === 0) throw new Error("Invalid DER length.");
        len = 0;
        for (let i = 0; i < num; i++) {
    ...
    if (pos + len > bytes.length) throw new Error("DER element exceeds input.");
    return {
        tag,
        contentStart: pos,
        contentEnd: pos + len,
