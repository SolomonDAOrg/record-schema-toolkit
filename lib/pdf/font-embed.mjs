...
/** @typedef {{ yMin: number; yMax: number }} TtfGlyphBounds */
 ...
 * @property {(codePoint: number) => TtfGlyphBounds | null} glyphBoundsForCodePoint - TrueType outline bounds in font units; null when unavailable or empty.
...
export function parseTtfFont(fontBytes) {
    ...
    const locationEntry = tables.get("loca");
    const outlineEntry = tables.get("glyf");
    const locationFormat = r16(fontBytes, headEntry.offset + 50);
    /** @type {Map<number, TtfGlyphBounds | null>} */
    const glyphBounds = new Map();
    /** @param {number} codePoint @returns {TtfGlyphBounds | null} */
    const glyphBoundsForCodePoint = (codePoint) => {
        const glyphIdentifier = glyphIdForCodePoint(codePoint);
        if (glyphBounds.has(glyphIdentifier))
            return glyphBounds.get(glyphIdentifier) ?? null;
        if (
            !locationEntry ||
            !outlineEntry ||
            (locationFormat !== 0 && locationFormat !== 1)
        )
            return null;
        const entrySize = locationFormat === 0 ? 2 : 4;
        if ((glyphIdentifier + 2) * entrySize > locationEntry.length)
            return null;
        const location = locationEntry.offset + glyphIdentifier * entrySize;
        const start =
            locationFormat === 0
                ? r16(fontBytes, location) * 2
                : r32(fontBytes, location);
        const end =
            locationFormat === 0
                ? r16(fontBytes, location + 2) * 2
                : r32(fontBytes, location + 4);
        const bounds =
            end - start >= 10 && end <= outlineEntry.length
                ? {
                      yMin: ri16(fontBytes, outlineEntry.offset + start + 4),
                      yMax: ri16(fontBytes, outlineEntry.offset + start + 8)
                  }
                : null;
        glyphBounds.set(glyphIdentifier, bounds);
        return bounds;
    };
    ...
    for (let c = 0; c < 256; c++) {
        charWidths[c] = widthForCodePoint(WINANSI_TO_UNICODE_OVERRIDES[c] ?? c);
    }
    ...
    if (os2Entry) {
        ...
        if (version >= 2 && os2Entry.length >= 90) {
            // OpenType OS/2 v2+: sxHeight at 86, sCapHeight at 88.
            const sCapHeight = ri16(fontBytes, os2Entry.offset + 88);
            if (sCapHeight > 0) {
 ...
 * @param {Pick<TtfParsed, "charWidths" | "postScriptName">} parsed
