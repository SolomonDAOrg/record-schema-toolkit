 ...
 * @param {string | Buffer | import("node:crypto").KeyObject} privateKey - Signing private key
...
export function buildCMS(signedBytes, privateKey, certDer, opts) {
    const keyObj =
        ...
        privateKey.type === "private"
            ? privateKey
            : createPrivateKey(/** @type {string} */ (privateKey));
    if (!["rsa", "ec", "ed25519"].includes(keyObj.asymmetricKeyType))
        throw new Error("Unsupported CMS signing key type.");
 ...
 * @property {string | Buffer | import("node:crypto").KeyObject} privateKey - signer private key, PEM (RSA, EC, or Ed25519)
 * @property {string | Buffer} certificate - signer certificate, PEM
 * @property {boolean} [existingField] - Require and sign the named visible widget
...
export function signPdf(bytes, options) {
    ...
    const key =
        ...
        "type" in options.privateKey &&
        options.privateKey.type === "private"
            ? options.privateKey
            : createPrivateKey(/** @type {string} */ (options.privateKey));
    if (!cert.checkPrivateKey(key))
        throw new Error("Signing certificate does not match the private key.");
    if (
        key.asymmetricKeyType === "rsa" &&
        key.asymmetricKeyDetails.modulusLength < 2048
    )
        throw new Error("RSA signing requires at least 2048 bits.");
    if (
        ...
        throw new Error("Unsupported EC signing curve.");
    const signingTime = options.signingTime ?? new Date();
    if (
        !Number.isFinite(signingTime.getTime()) ||
        signingTime < new Date(cert.validFrom) ||
        signingTime > new Date(cert.validTo)
    )
        throw new Error(
            "Certificate is not valid at the claimed signing time."
        );
    if (
        signingTime.getUTCFullYear() < 1950 ||
        signingTime.getUTCFullYear() > 2049
    )
        throw new Error(
            "This CMS profile supports signing times from 1950 through 2049."
        );

    const ed = loadPdf(bytes);
    ed.addSignatureField({
        fieldName: options.fieldName,
        existingField: options.existingField,
        name: options.name,
