 ...
 * @property {string} [fontName] - Registered font alias used by widget text and editing.
...
export class PdfDocumentBuilder {
    ...
    build() {
        ...
        const fieldAppearanceIds = new Map();
        ...
        for (let i = 0, len = this.pages.length; i < len; i++) {
            ...
            for (let j = 0, jlen = pageData.annotations.length; j < jlen; j++) {
                ...
                fieldAppearanceIds.set(annotIds[j], nextObjId++);
        ...
        for (let i = 0, len = pageObjects.length; i < len; i++) {
            ...
            for (let j = 0, jlen = annotIds.length; j < jlen; j++) {
                ...
                const annotation = pageData.annotations[j];
                const appearanceId = fieldAppearanceIds.get(annotId);
                emitObject(
                    annotId,
                    annotation.type === "form"
                        ? this.createFormWidgetAnnotation(annotation, pageId, appearanceId)
        ...
        for (let i = 0; i < pageObjects.length; i++) {
            for (let j = 0; j < pageObjects[i].annotIds.length; j++) {
                const annotation = this.pages[i].annotations[j];
                const appearanceId = fieldAppearanceIds.get(pageObjects[i].annotIds[j]);
                if (annotation.type === "form" && appearanceId !== undefined) {
                    const fontName = annotation.fontName ?? "Helvetica";
                    const fontResource = this.namedFonts.get(fontName) ?? this.namedFonts.get("Helvetica");
                    const fontObject = fontResourceMap[fontResource];
                    if (fontObject === undefined) throw new Error("Unregistered form field font: " + fontName);
                    emitStream(appearanceId, {
                        Type: "/XObject", Subtype: "/Form", FormType: "1",
                        BBox: formatArray([0, 0, annotation.width, annotation.height].map(String)),
                        Resources: formatDictionary({ Font: formatDictionary({ [fontResource]: formatRef(fontObject) }) })
                    }, encodeUtf8(this.createFormFieldAppearance(annotation)));
                }
            }
        }
     ...
     * @param {Record<string, number>} [fontResources]
     * @returns {string}
     */
    createAcroForm(fieldObjectIds, helveticaObjectId, hasSignatureField, fontResources = {}) {
        /** @type {Record<string, string>} */
        const dict = {
            ...
            DR: formatDictionary({
                Font: formatDictionary({
                    ...
                    ...Object.fromEntries(Object.entries(fontResources).map(([name, id]) => [name, formatRef(id)]))
                })
            }),
     ...
     * @returns {string}
     */
    createFormWidgetAnnotation(annotation, pageId, appearanceId) {
        const x1 = annotation.x;
        const y1 = annotation.y;
        const x2 = annotation.x + annotation.width;
        const y2 = annotation.y + annotation.height;
        ...
        if (annotation.fieldType === "text" && annotation.multiline) {
            fieldFlags = fieldFlags | 4096;
        }
        ...
        if (appearanceId !== undefined) {
            dict.AP = formatDictionary({ N: formatRef(appearanceId) });
        }
        ...
        if (annotation.fieldType === "text") {
            const fontSize =
                annotation.fontSize && annotation.fontSize > 0
                    ? annotation.fontSize
                    : 10;
            const fontResource = this.namedFonts.get(annotation.fontName ?? "Helvetica") ?? this.namedFonts.get("Helvetica");
            dict.DA = `(${escapePdfString(`/${fontResource} ${fontSize} Tf 0 g`)})`;
            dict.Q = "0";
     ...
     * @returns {string}
     */
    createFormFieldAppearance(annotation) {
        if (annotation.fieldType === "signature" || !annotation.value) return "q\nQ\n";
        const fontSize = annotation.fontSize && annotation.fontSize > 0 ? annotation.fontSize : 10;
        const step = fontSize * 1.2;
        const fontName = annotation.fontName ?? "Helvetica";
        const fontResource = this.namedFonts.get(fontName) ?? this.namedFonts.get("Helvetica");
        const lines = [];
        for (const paragraph of annotation.value.split(/\r\n|\r|\n/)) {
            let line = "";
            for (const word of paragraph.split(/\s+/)) {
                const candidate = line ? `${line} ${word}` : word;
                if (annotation.multiline && line && measureTextWidth(candidate, fontName, fontSize) > annotation.width - 4) {
                    lines.push(line);
                    line = word;
                } else line = candidate;
            }
        ...
        const baseline = formFieldTextBaseline(annotation.height, fontSize, annotation.multiline);
        return `q\n0 0 ${annotation.width} ${annotation.height} re W n\nBT\n/${fontResource} ${fontSize} Tf\n0 g\n${step} TL\n2 ${baseline} Td\n` +
            (annotation.multiline ? lines : [lines.join(" ")]).map((line, index) => `${index ? "T*\n" : ""}(${escapePdfString(line)}) Tj`).join("\n") + "\nET\nQ\n";
    }
}

/**
 * Baseline within a text widget, shared by its appearance and adjacent labels.
 * @param {number} height
 * @param {number} fontSize
 * @param {boolean} [multiline]
 * @returns {number}
 */
export function formFieldTextBaseline(height, fontSize, multiline = false) {
    return multiline ? height - fontSize : Math.max(0, (height - fontSize) / 2 + fontSize * 0.2);
}
