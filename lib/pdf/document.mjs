...
export class PdfDocumentBuilder {
    ...
    measureEmbeddedFontText(resourceName, text, fontSize) {
        ...
        const { charWidths } = info.metrics;
        let width = 0;
        for (let i = 0, len = text.length; i < len; i++) {
            const code = text.charCodeAt(i);
            width = width + (code < 256 ? charWidths[code] : 500);
        }
        return (width / 1000) * fontSize;
    }
    ...
    build() {
        ...
        writeStr(
            formatIndirectObject(
                resourcesId,
                createResources(fontResourceMap, xObjectResourceMap)
            )
        );
        ...
        for (let i = 0, len = embeddedFontEntries.length; i < len; i++) {
            ...
            writeStr(
                formatIndirectObject(
                    ef.fontDictId,
                    buildTtfFontDict(ef.metrics, ef.descriptorId)
                )
            );
            ...
            const ttfDict = formatDictionary({
                Length: String(ttfBytes.length),
                Length1: String(ttfBytes.length)
            });
            writeStr(`${ef.fileStreamId} 0 obj\n${ttfDict}\nstream\n`);
            write(ttfBytes);
            writeStr("\nendstream\nendobj\n");
        }
        ...
        for (let i = 0, len = imageEntries.length; i < len; i++) {
            ...
            if (img.format === "jpeg") {
                ...
                objectOffsets.push(currentOffset);
                const fullDict = formatDictionary(buildJpegXObjectDict(parsed));
                writeStr(`${img.xObjectId} 0 obj\n${fullDict}\nstream\n`);
                write(parsed.streamData);
                writeStr("\nendstream\nendobj\n");
            } else {
                ...
                if (img.smaskId !== undefined && parsed.smaskData) {
                    objectOffsets.push(currentOffset);
                    const smaskDict = formatDictionary(
                        buildSmaskXObjectDict(
                            parsed.width,
                            parsed.height,
                    ...
                    writeStr(`${img.smaskId} 0 obj\n${smaskDict}\nstream\n`);
                    write(parsed.smaskData);
                    writeStr("\nendstream\nendobj\n");
                }
            }
        }
