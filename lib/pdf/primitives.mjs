 ...
 * @returns {Uint8Array}
 */
export function createPdfHeader(version) {
    // Include binary comment to indicate binary content
    return encodeUtf8((version || PDF_VERSION) + "\n%\x80\x81\x82\x83\n");
}
 ...
 * @param {string} [idHex] - Optional document /ID (hex of 16 bytes); written as [<id> <id>]
 * @param {number} [encryptRef] - Optional /Encrypt object ID
 * @returns {string}
 */
export function createTrailer(
    ...
    if (encryptRef !== undefined) {
        entries["Encrypt"] = formatRef(encryptRef);
    }
    if (infoRef !== undefined) {
        entries["Info"] = formatRef(infoRef);
    }
    if (idHex !== undefined) {
        entries["ID"] = `[<${idHex}> <${idHex}>]`;
    }
    const dict = formatDictionary(entries);
    return `trailer\n${dict}\nstartxref\n${xrefOffset}\n${PDF_EOF}\n`;
}
 ...
 * @param {number} [acroFormRef] - Optional AcroForm object ID
 * @param {Record<string, string>} [extraEntries] - Additional catalog entries (e.g. Names, AF)
 * @returns {string}
 */
export function createCatalog(
    ...
    if (acroFormRef != null) {
        entries.AcroForm = formatRef(acroFormRef);
    }
    if (extraEntries) {
        const keys = Object.keys(extraEntries);
        for (let i = 0, len = keys.length; i < len; i++) {
            entries[keys[i]] = extraEntries[keys[i]];
        }
    }
...
/**
 * Creates a resources object with fonts and optional XObjects (images, forms)
 * @param {Record<string, number>} fonts - Font resource name to object ID mapping
 * @param {Record<string, number>} [xObjects] - XObject resource name to object ID mapping
 * @param {Record<string, string>} [shadings]
 * @param {Record<string, string>} [extGStates]
 * @param {Record<string, string>} [colorSpaces]
 * @returns {string}
 */
export function createResources(
    ...
    /** @type {Record<string, string>} */
    const entries = { Font: formatDictionary(fontEntries) };

    if (xObjects && Object.keys(xObjects).length > 0) {
        /** @type {Record<string, string>} */
        const xoEntries = {};
        const xoNames = Object.keys(xObjects);
        for (let i = 0, len = xoNames.length; i < len; i++) {
            xoEntries[xoNames[i]] = formatRef(xObjects[xoNames[i]]);
        }
        entries["XObject"] = formatDictionary(xoEntries);
    }

    if (shadings && Object.keys(shadings).length > 0) {
        /** @type {Record<string, string>} */
        const shEntries = {};
        const shNames = Object.keys(shadings);
        for (let i = 0, len = shNames.length; i < len; i++) {
            // shadings values are inline dict strings, not object IDs
            shEntries[shNames[i]] = shadings[shNames[i]];
        }
        entries["Shading"] = formatDictionary(shEntries);
    }

    if (extGStates && Object.keys(extGStates).length > 0) {
        /** @type {Record<string, string>} */
        const gsEntries = {};
        const gsNames = Object.keys(extGStates);
        for (let i = 0, len = gsNames.length; i < len; i++) {
            // ExtGState values are inline dict strings
            gsEntries[gsNames[i]] = extGStates[gsNames[i]];
        }
        entries["ExtGState"] = formatDictionary(gsEntries);
    }

    if (colorSpaces && Object.keys(colorSpaces).length > 0) {
        /** @type {Record<string, string>} */
        const csEntries = {};
        const csNames = Object.keys(colorSpaces);
        for (let i = 0, len = csNames.length; i < len; i++) {
            // color space values are inline arrays/dicts
            csEntries[csNames[i]] = colorSpaces[csNames[i]];
        }
        entries["ColorSpace"] = formatDictionary(csEntries);
    }
