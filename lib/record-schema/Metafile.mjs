...
export class Metafile {
    ...
    getPrimaryDocumentPath() {
        ...
        const p =
            /** @type {DocumentRef[] | DocumentRef | null | undefined} */ (arr);
        if (isString(p)) {
            return p;
        }
    ...
    /**
     * Resolve record and document rendering metadata from this manifest.
     * Paths are relative to the record directory; an omitted path selects the packet.
     * @param {{path?: string, doc_type?: string | null, record_id?: string}} [options]
     * @returns {{path: string, label?: string, short_label?: string | null, record_id: string, document_id: string, version: string | undefined, doc_type: string | null, render_profile_id: string | undefined}}
     */
    resolveRenderMetadata(options = {}) {
        const record_id = this.data.id || options.record_id || "";
        const normalize = (/** @type {string} */ value) =>
            value.replace(/\\/g, "/").replace(/^\.\//, "");
        /** @type {DocumentRef | AssemblyPackEntry | AssemblyPacketInfo} */
        let reference = this.data.assembly?.packet || { path: "" };
        if (options.path !== undefined) {
            const path = normalize(options.path);
            reference = { path };
            for (const value of Object.values(this.data.documents || {})) {
                for (const entry of Array.isArray(value) ? value : [value]) {
                    if (
                        isObject(entry) &&
                        typeof entry.path === "string" &&
                        normalize(entry.path) === path
                    ) {
                        reference = { ...reference, ...entry };
                    }
                }
            }
            for (const entry of this.data.assembly?.pack || []) {
                if (normalize(entry.path) === path)
                    reference = { ...reference, ...entry };
            }
        }
        const stem = basename(options.path || "").replace(/\.[^.]+$/, "");
        const canonical = stem.match(/^[A-Z]{2,5}-\d+_([A-Z]{2,5})(.*)$/);
        const doc_type =
            ("doc_type" in reference ? reference.doc_type : null) ||
            options.doc_type ||
            canonical?.[1] ||
            null;
        const version =
            ...
            this.data.version;
        let document_id = reference.document_id;
        if (!document_id && options.path === undefined)
            document_id = this.data.document?.document_id || record_id;
        if (!document_id) {
            const suffix = canonical ? canonical[2] : stem ? "-" + stem : "";
            document_id = record_id + (doc_type ? "_" + doc_type : "") + suffix;
        }
        return {
            ...reference,
            path: normalize(reference.path),
            record_id,
            document_id: version ? document_id + "/" + version : document_id,
            version,
            doc_type,
            render_profile_id:
                reference.render_profile_id || this.getDefaultRenderProfile()
        };
    }
