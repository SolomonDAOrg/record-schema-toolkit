...
import { evaluateDigest } from "./Digest.mjs";
...
export function materializeAssertionsWithContext(
    ...
    const selectedRules = pack.getRules();
    const rules = [
        ...selectedRules.filter((rule) => rule.kind !== "digest"),
        ...selectedRules.filter((rule) => rule.kind === "digest")
    ];
    ...
    for (let i = 0, len = rules.length; i < len; i++) {
        ...
        try {
            if (rule.kind === "digest") {
                if (rule.materialize?.operation !== "digest_manifest")
                    throw new Error(
                        "digest materializer requires digest_manifest"
                    );
                const result = evaluateDigest(
                    rule,
                    new CorpusIndex(rootDirectory, stagedContents),
                    pack.resolved,
                    currentContent(rule.manifest),
                    stagedContents
                );
                stage(rule.id, result.file, result.content);
                continue;
            }
            if (rule.kind === "derive") {
                materializeDeriveRule(rule, engine, currentContent, stage);
                continue;
            }
            if (rule.kind === "format") {
 ...
 * @returns {void}
 */
function materializeFormatRule(rule, index, pack, currentContent, stage) {
    if (!isPlainObject(rule.materialize)) {
        throw new Error("materialize must be a mapping");
    }
    const specification = /** @type {Record<string, unknown>} */ (
...
function rewriteYamlInlineMappingField(
    ...
    for (let i = 0, len = segments.length; i < len; i++) {
        ...
        const colon = findTopLevelColon(inside, segment.start, segment.end);
        if (colon === -1) {
            continue;
        }
        const key = unquoteYamlKey(inside.slice(segment.start, colon).trim());
        if (key !== field) {
            continue;
        }
        ...
        rewrittenInside =
            inside.slice(0, valueStart) + serialized + inside.slice(valueEnd);
        replaced = true;
        break;
    }
    ...
    if (!replaced) {
        ...
        const leading =
            body.trim().length === 0 && inside.length > 0
                ? inside.slice(0, inside.length - inside.trimStart().length)
                : "";
        if (body.trim().length === 0) {
            rewrittenInside = `${leading} ${field}: ${serialized}${
                trailing || " "
            }`;
        } else {
    ...
    const content =
        text.slice(0, open + 1) + rewrittenInside + text.slice(close);
...
function renderYamlInlineSequence(field, entries) {
    ...
    for (let i = 0, len = entries.length; i < len; i++) {
        ...
        for (let j = 0, count = names.length; j < count; j++) {
            fields.push(`${names[j]}: ${serializeYamlScalar(entry[names[j]])}`);
        }
...
function containedOutputPath(rootDirectory, file) {
    if (isAbsolute(file))
        throw new Error(`output must be repository-relative: ${file}`);
    ...
    for (const part of rel.split(sep)) {
        ...
        try {
            if (lstatSync(current).isSymbolicLink())
                throw new Error(`output crosses a symbolic link: ${file}`);
        } catch (error) {
