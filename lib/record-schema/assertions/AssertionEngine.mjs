 ...
 *
 * Fifteen rule kinds, each describing a defect shape rather than a corpus
...
export class AssertionEngine {
    ...
    _runDigest(rule, findings) {
        const result = evaluateDigest(rule, this.index, this.pack);
        this._countSelection(result.count, result.count);
        for (const issue of result.issues)
            findings.push({
                ...this._finding(rule, null, {
                    "#file": result.file,
                    "#reason": issue
                }),
                file: result.file,
                message: (rule.message || rule.id) + ": " + issue
            });
    }
