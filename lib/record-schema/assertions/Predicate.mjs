...
export function evaluatePredicate(predicate, value, context = {}) {
    ...
    for (let i = 0, len = keys.length; i < len; i++) {
        ...
        if (key === "expr") {
            if (!evaluateExpressionClause(clause.expr, value, context)) {
                return false;
            }
            continue;
        }
 ...
 * @returns {boolean}
 */
function evaluateExpressionClause(clause, value, context) {
    ...
    const bindings = expressionBindings(context);

    for (let i = 0, len = names.length; i < len; i++) {
        const name = names[i];
        const expression = bindSpec[name];
        if (expression.startsWith("#")) {
            bindings[name] = context[expression] ?? null;
            continue;
        }
    ...
    for (let i = 0, len = operators.length; i < len; i++) {
        ...
        if (CARDINALITY_OPERATORS.has(operator)) {
            if (
                !evaluateCardinality(
                    operator,
                    resolveOperand(operand, context),
                    selected
                )
            ) {
                return false;
        ...
        const resolvedOperand = resolveOperand(operand, context);
        /** @type {(item: unknown) => boolean} */
        const test = (item) =>
            evaluateOperator(operator, resolvedOperand, item);
        const held =
            mode === "any"
...
/**
 * Make selector bindings available to expression predicates without exposing
 * the `#` sigil to the expression grammar. Both spellings are retained so a
 * pack can deliberately bind an ordinary name beginning with no sigil.
 *
 * @param {Record<string, unknown>} context
 * @returns {Record<string, unknown>}
 */
function expressionBindings(context) {
    /** @type {Record<string, unknown>} */
    const bindings = {};
    const names = Object.keys(context);
    for (let i = 0, len = names.length; i < len; i++) {
        const name = names[i];
        bindings[name] = context[name];
        if (name.startsWith("#") && name.length > 1) {
            bindings[name.slice(1)] = context[name];
        }
    }
    return bindings;
}
...
/**
 * A predicate operand may name another selector binding. This is the
 * declarative equivalent of comparing two fields in a joined row, and keeps
 * dynamic equality out of corpus-specific rule code.
 *
...
function resolveOperand(operand, context) {
    if (
        ...
        operand.startsWith("#") &&
        Object.prototype.hasOwnProperty.call(context, operand)
    ) {
        return context[operand];
    }
    if (Array.isArray(operand)) {
        const resolved = [];
        for (let i = 0, len = operand.length; i < len; i++) {
            resolved.push(resolveOperand(operand[i], context));
        }
        return resolved;
    }
    return operand;
...
export function looseEqual(left, right) {
    ...
    const leftInteger = coerceIntegerExact(left);
    const rightInteger = coerceIntegerExact(right);
 ...
 * @returns {bigint | null}
 */
function coerceIntegerExact(value) {
    ...
    const match = /^([+-]?)(0[xX][0-9a-fA-F]+|0[bB][01]+)$/.exec(text);
    if (match === null) {
        return null;
    }
