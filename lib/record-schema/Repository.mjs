...
import { UPSTREAM_FILENAMES } from "./constants/constants.mjs";
...
export class Repository {
    ...
    static discoverRepository(startDir, options = {}) {
        ...
        if (result.profile_path && (result.pack_paths.length === 0 || result.render_pack_paths.length === 0)) {
            const profile = Profile.loadIfExists(result.profile_path);
            if (profile && profile.hasPackPaths()) {
                const profilePackPaths = result.pack_paths.length === 0
                    ? profile.getPackPaths("formatting")
                    : [];
                trace(
                ...
                if (result.render_pack_paths.length === 0) {
                    for (const renderPath of profile.getPackPaths("render")) {
                        if (existsSync(resolve(result.root_dir, renderPath))) {
                            result.render_pack_paths.push(renderPath);
                        }
                    }
                }
            } else {
