...
export class Repository {
    ...
    static _hydrateFromDiscovery(repo, discovery, verbose = false) {
        ...
        // 5. Render packs (try repo root, then toolkit schema roots)
        if (discovery.render_pack_paths.length > 0) {
            ...
            for (let r = 0, rLen = roots.length; r < rLen; r++) {
                ...
                for (
                    ...
                    if (
                        !existsSync(
                            resolve(root, discovery.render_pack_paths[i])
                        )
                    ) {
                        ok = false;
                        break;
                ...
                if (ok) {
                    const res = RenderPack.loadMerged(
                        root,
                        discovery.render_pack_paths
                    );
                    repo._render_policy = res.policy;
                    if (res.packet_config) {
                        repo._packet_config = res.packet_config;
                    }
                    break;
                }
            }
