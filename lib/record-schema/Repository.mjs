...
import { readFileSync, readdirSync, existsSync, writeFileSync, lstatSync, realpathSync } from "node:fs";
import { resolve, dirname, basename, extname, relative, isAbsolute, sep } from "node:path";
...
export class Repository {
            ...
            if (upstream_path) {
                ...
                trace(
                    `  upstream.registry_paths: ${
                        upstreamConfig.registry_paths && upstreamConfig.registry_paths.length > 0
                ...
                if (upstreamConfig.registry_paths && upstreamConfig.registry_paths.length > 0) {
    ...
    static _discoverByConvention(root_dir, result, trace) {
        ...
        if (!result.registry_path && (!result.registry_paths || result.registry_paths.length === 0)) {
        ...
        if (discovery.pack_paths.length > 0) {
            ...
            for (let r = 0, rLen = roots.length; r < rLen; r++) {
                ...
                if (ok) {
                    ...
                    repo._loaded_pack_paths = res.packs.map(
                        (p) => {
                ...
                for (let c = 0, cLen = candidates.length; c < cLen; c++) {
                    ...
                    try {
                        ...
                        if (text.includes("record-schema-formatting-pack")) {
                            ...
                            trace(
                                `hydrate: found formatting pack in toolkit fallback: ${candidates[c].abs_path}`
                            );
                        }
                    } catch {
                        // skip unreadable
            ...
            if (fallbackFmtPacks.length > 0) {
                ...
                trace(
                    `hydrate: loaded ${fallbackFmtPacks.length} formatting packs from toolkit fallback (root=${schemaRoot})`
                );
            } else {
                trace(
                    `hydrate: no formatting packs (including toolkit fallback)`
                );
            }
        }
    ...
    getSchemaMaterialRoots() {
        ...
        const candidates = [
            this.root_dir,
            toolkitDir,
            resolve(toolkitDir, "../record-schema")
        ];
        /** @type {string[]} */
        const roots = [];
        const seen = new Set();
    ...
    loadSchemaMaterial(relativePath) {
        ...
        for (let i = 0, len = roots.length; i < len; i++) {
            const schema = Schema.loadIfExists(resolve(roots[i], relativePath));
            if (schema) {
                return schema;
            }
        }
    ...
    resolveContainedSchemaMaterialPath(relativePath) {
        if (!isString(relativePath) || relativePath.trim().length === 0 || isAbsolute(relativePath)) {
            return null;
        }
        const absolutePath = resolve(this.root_dir, relativePath);
        const relativePathFromRoot = relative(this.root_dir, absolutePath);
        if (
    ...
    isContainedRegularSchemaMaterialPath(absolutePath) {
        ...
        try {
            ...
            const repositoryRealPath = realpathSync(this.root_dir);
            const materialRealPath = realpathSync(absolutePath);
            const relativeRealPath = relative(repositoryRealPath, materialRealPath);
            return (
    ...
    validateConfiguredSchemaMaterials() {
        ...
        const validateSchemaPath = (configuredPath) => {
            const absolutePath = this.resolveContainedSchemaMaterialPath(configuredPath);
            if (!absolutePath) {
                issues.push({
                    severity: "error",
                    code: "configured.schema.path",
                    message: `Configured schema path must remain inside the repository: ${configuredPath}`,
                    file: configuredPath
                });
                return;
            }
            ...
            if (!this.isContainedRegularSchemaMaterialPath(absolutePath)) {
                issues.push({
                    severity: "error",
                    code: "configured.schema.path",
                    message: `Configured schema path must resolve to a contained regular non-symlink file: ${configuredPath}`,
                    file: configuredPath
                });
            }
        };

        const overlayPaths = this._profile.getMetaOverlaySchemaPaths();
        for (let i = 0, len = overlayPaths.length; i < len; i++) {
            validateSchemaPath(overlayPaths[i]);
        }

        const bindings = this._profile.getStructuredDocumentSchemas();
    ...
    getRecordMetaSchemas() {
        ...
        for (let i = 0, len = overlayPaths.length; i < len; i++) {
            ...
            const absolutePath = this.resolveContainedSchemaMaterialPath(overlayPath);
            if (!absolutePath || !this.isContainedRegularSchemaMaterialPath(absolutePath)) {
                continue;
            }
    ...
    getLoadedRenderPackRoots() {
        if (this._loaded_render_pack_roots && this._loaded_render_pack_roots.length > 0) {
    ...
    static extractRenderSelection(metafile_or_data) {
        const raw =
            metafile_or_data instanceof Metafile
                ...
                ? typeof /** @type {{ data?: unknown }} */ (metafile_or_data).data === "object" &&
                  /** @type {{ data?: unknown }} */ (metafile_or_data).data !== null
        ...
        const extensions_record = /** @type {Record<string, unknown>} */ (raw_record.extensions || {});
        const assembly_record = /** @type {Record<string, unknown>} */ (raw_record.assembly || {});
        const packet_record = /** @type {Record<string, unknown>} */ (assembly_record.packet || {});

        const extensions_rendering = parseSelection(extensions_record.rendering);
        ...
        if (!merged.family) {
            ...
            const authority_record = /** @type {Record<string, unknown>} */ (raw_record.authority || {});
            ...
            const document_record = /** @type {Record<string, unknown>} */ (raw_record.document || {});
            if (typeof document_record.title === "string") {
                family_hints.push(document_record.title);
            }

            const documents_record = /** @type {Record<string, unknown>} */ (raw_record.documents || {});
            const primary_record = /** @type {Record<string, unknown>} */ (documents_record.primary || {});
    ...
    getResolvedRenderPack(metafile_or_data = undefined) {
        ...
        if (selection.pack_paths.length > 0) {
            ...
            for (
                let i = 0, len = candidate_roots.length;
                i < len;
                i++
            ) {
                ...
                const merged_result = RenderPack.loadMerged(root, selection.pack_paths);
        ...
        const withSupportPacks = (packs, selected) => {
            ...
            for (let i = 0, len = selected.length; i < len; i++) {
                ...
                if (typeof next_pack?.source_path === "string" && next_pack.source_path.length > 0) {
            ...
            for (let i = 0, len = packs.length; i < len; i++) {
                ...
                const source_path = typeof next_pack?.source_path === "string" ? resolve(next_pack.source_path) : "";
                const id_label = normalizeKey(next_pack.getId());
                const descriptor = `${id_label} ${basename(next_pack.source_path || "")} ${next_pack.getDescription() || ""}`.toLowerCase();
                const is_support_pack =
                    descriptor.includes("base") ||
                    descriptor.includes("common") ||
                    descriptor.includes("shared");
                if (selected_sources.has(source_path) || selected_ids.has(id_label) || is_support_pack) {
    ...
    validateStructuredDocuments(record) {
        ...
        for (let i = 0, len = bindings.length; i < len; i++) {
            const binding = bindings[i];
            const schemaPath = this.resolveContainedSchemaMaterialPath(binding.schema_path);
            if (!schemaPath || !this.isContainedRegularSchemaMaterialPath(schemaPath)) {
                continue;
            }
            ...
            for (let j = 0, jLen = documents.length; j < jLen; j++) {
                ...
                if (binding.extensions.length > 0 && !binding.extensions.includes(extension)) {
                    continue;
                }
                matched++;
                ...
                try {
                    if (extension === "json") {
                        ...
                        value = parseYaml(readFileSync(sourcePath, "utf8"), { filename: sourcePath });
                    } else {
                        issues.push({
                            severity: "error",
                    ...
                    issues.push({
                        ...
                        message: error instanceof Error ? error.message : "Structured document parse failed",
                        file: this.getRelativePath(sourcePath)
                    });
                    continue;
                }
            ...
            const minimumCount = binding.min_count ?? (binding.required ? 1 : 0);
