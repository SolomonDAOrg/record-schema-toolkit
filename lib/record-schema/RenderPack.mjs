...
export class RenderPack {
    ...
    static loadMerged(root_dir, pack_paths, search_roots = []) {
        ...
        for (let i = 0, len = ordered.length; i < len; i++) {
            ...
            merged = RenderPack._mergePolicy(
                merged,
                pack._getResolvedDocumentPolicies()
            );
    ...
    static mergePacks(packs) {
        ...
        for (let i = 0, len = packs.length; i < len; i++) {
            merged = RenderPack._mergePolicy(
                merged,
                packs[i]._getResolvedDocumentPolicies()
            );
    ...
    resolveCoverConfig(meta, profile_cover) {
        ...
        const watermark = {
            ...
            text: layeredValue(wm_layers, "text", "string", "DRAFT") ?? "DRAFT",
        ...
        return {
            ...
            reserve_header_footer_space: reserve_header_footer_space ?? false,
            watermark,
            cover_layout,
            font_roles:
    ...
    resolvePackEntries(meta, options) {
        ...
        const assembly = /** @type {Record<string, unknown>} */ (
            meta_record.assembly || {}
        );
        const documents = /** @type {Record<string, unknown>} */ (
            meta_record.documents || {}
        );
    ...
    static _normalizePackArray(raw_pack, options) {
        ...
        for (let i = 0, len = raw_pack.length; i < len; i++) {
            ...
            entries.push({
                ...
                render_profile_id: entry.render_profile_id,
        ...
        entries.sort(
            (/** @type {PackEntry} */ a, /** @type {PackEntry} */ b) =>
        ...
        for (let i = 0, len = primary_arr.length; i < len; i++) {
            ...
            if (typeof path === "string" && path.length > 0) {
                entries.push({
                    path,
                    precedence: i * 10,
                    doc_type: isObject(ref)
                        ? stringOr(ref.doc_type, null)
                        : null,
                    render_profile_id: isObject(ref)
                        ? stringOr(ref.render_profile_id, undefined)
                        : undefined,
                    document_id: isObject(ref)
                        ? stringOr(ref.document_id, undefined)
                        : undefined,
                    version: isObject(ref)
                        ? stringOr(ref.version, undefined)
                        : undefined,
                    label: isObject(ref)
                        ? stringOr(
                              /** @type {Record<string, unknown>} */ (ref)
                                  .label,
                              undefined
                          ) ?? null
                        : null,
    ...
    resolveDocumentKind(meta, pack_entries) {
        ...
        const extensions_record = /** @type {Record<string, unknown>} */ (
            meta_record.extensions || {}
        );
        const formatting_record = /** @type {Record<string, unknown>} */ (
            extensions_record.formatting || {}
        );
        ...
        const title = (
            typeof meta_record.title === "string" ? meta_record.title : ""
        ).toLowerCase();
    ...
    static _extractPrimaryPath(meta) {
        ...
        if (primary_arr && !Array.isArray(primary_arr)) {
            ...
            if (isObject(primary_arr)) {
                return (
                    stringOr(
                        /** @type {Record<string, unknown>} */ (primary_arr)
                            .path,
                        undefined
                    ) ?? ""
                );
    ...
    static _resolveCoverConfigLayer(raw, defaults) {
        ...
        return {
            ...
            draft_watermark: {
                ...
                text:
                    stringOr(rw?.text, dw.text ?? undefined) ??
                    "DRAFT DOCUMENT",
     ...
     */
    resolveRenderProfile(profile_id, ancestors = new Set()) {
        if (ancestors.has(profile_id)) {
            throw new Error(
                `Render profile inheritance cycle: ${[
                    ...ancestors,
                    profile_id
                ].join(" -> ")}`
            );
        ...
        const parent_profile = this.resolveRenderProfile(
            profile.extends,
            new Set([...ancestors, profile_id])
        );
        if (!parent_profile) {
            throw new Error(
                `Unknown parent render profile: ${profile.extends}`
            );
        }
    ...
    /**
     * Bind pack-relative assets before merging policies from different source files.
     * @returns {RenderDocumentPolicy}
     */
    _getResolvedDocumentPolicies() {
        const policy = structuredClone(this.getDocumentPolicies());
        /** @template {RenderTarget | RulesetRender} T @param {T} configuration @returns {T} */
        const resolveAssets = (configuration) => {
            const fonts = configuration.embedded_fonts;
            if (fonts?.resolve_relative_to === "pack" && this.source_path) {
                const baseDirectory = stringOr(
                    fonts.base_dir,
                    dirname(this.source_path)
                );
                const families =
                    /** @type {Record<string, {faces?: Record<string, {path?: string}>}>} */ (
                        fonts.families ?? {}
                    );
                for (const family of Object.values(families)) {
                    for (const face of Object.values(family.faces ?? {})) {
                        if (face.path)
                            face.path = resolve(baseDirectory, face.path);
                    }
                }
            }
            return this._resolveLetterheadLogoPath(configuration);
        };
        for (const [name, target] of Object.entries(policy.targets ?? {}))
            policy.targets[name] = resolveAssets(target);
        for (const profile of Object.values(policy.render_profiles ?? {})) {
            if (profile.overrides)
                profile.overrides = resolveAssets(profile.overrides);
        }
        for (const rule of policy.rulesets ?? [])
            rule.render = resolveAssets(rule.render);
        if (policy.defaults) policy.defaults = resolveAssets(policy.defaults);
        return policy;
    }
    ...
    getEffectiveTargetConfig(profile_id) {
        ...
        if (isObject(effective_target)) {
            const embedded_fonts = isObject(effective_target.embedded_fonts)
                ? /** @type {Record<string, unknown>} */ (
                      effective_target.embedded_fonts
                  )
                : null;
        ...
        return {
            target: this._resolveLetterheadLogoPath(effective_target),
            profile
        };
    }

    /**
     * Resolve local artwork at the pack boundary, including ruleset overrides.
     * @template {RenderTarget | RulesetRender} T
     * @param {T} configuration
     * @returns {T}
     */
    _resolveLetterheadLogoPath(configuration) {
        const composition = configuration.page_composition;
        const logo = composition?.letterhead_logo;
        if (
            !logo ||
            logo.enabled === false ||
        ...
        const baseDirectory = this.source_path
            ? dirname(this.source_path)
            : process.cwd();
        return {
            ...configuration,
            page_composition: {
                ...
                letterhead_logo: {
                    ...logo,
                    path: resolve(baseDirectory, logo.path)
                }
            }
        };
    }
     ...
     *
     * @param {{ rel_path: string, doc_type: string, ext: string, render_profile_id?: string }} file
    ...
    resolveForFile(file) {
        ...
        if (!render_profile_id) {
            return this._resolveLetterheadLogoPath(effective);
        }

        const target_config = this.getEffectiveTargetConfig(render_profile_id);
        if (!target_config) {
            throw new Error(
                `Unknown render profile or target: ${render_profile_id}`
            );
        }

        // Profile/base target first, explicit ruleset render second.
        const configuration = file.render_profile_id
            ? RenderPack._mergeRender(effective, target_config.target)
            : RenderPack._mergeRender(target_config.target, effective);
        return this._resolveLetterheadLogoPath(configuration);
    }
            ...
            if (!isObject(r.selectors)) {
                ...
                if (
                    ...
                    for (
                        ...
                        if (
                            sel.doc_types[j] !== "PACKET" &&
                            !DOC_TYPE_PATTERN.test(sel.doc_types[j])
                        ) {
                            errors.push({
    ...
    _validateTargets(targets, errors) {
        ...
        for (const key of Object.keys(targets)) {
            const targets_record = /** @type {Record<string, unknown>} */ (
                targets
            );
            ...
            const target_record = /** @type {Record<string, unknown>} */ (
                target
            );
    ...
    _validateEmbeddedFonts(path, embedded_fonts, errors) {
        ...
        const embedded_fonts_record = /** @type {Record<string, unknown>} */ (
            embedded_fonts
        );
        ...
        const families = isObject(embedded_fonts_record.families)
            ? /** @type {Record<string, unknown>} */ (
                  embedded_fonts_record.families
              )
            : {};
        for (const familyKey of Object.keys(families)) {
            ...
            const family_record = /** @type {Record<string, unknown>} */ (
                family
            );
            ...
            for (const faceKey of Object.keys(faces)) {
                ...
                const face_record = /** @type {Record<string, unknown>} */ (
                    face
                );
                const hasPath =
                    typeof face_record.path === "string" &&
                    face_record.path.length > 0;
                const hasFile =
                    typeof face_record.file === "string" &&
                    face_record.file.length > 0;
                const google_font = isObject(face_record.google_font)
                    ? /** @type {Record<string, unknown>} */ (
                          face_record.google_font
                      )
                    : isObject(face_record.googleFont)
                    ? /** @type {Record<string, unknown>} */ (
                          face_record.googleFont
                      )
                    : null;
                const hasGoogleFont =
                    !!google_font &&
    ...
    _validateRenderProfiles(profiles, errors) {
        ...
        const profiles_record = /** @type {Record<string, unknown>} */ (
            profiles
        );
