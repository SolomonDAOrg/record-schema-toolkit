...
import { MarkdownToAstConverter } from "../ast/converters/MarkdownToAstConverter.mjs";
...
/** @typedef {Pick<PackEntry, "path"> & Partial<PackEntry>} DocumentRenderEntry */
/** @typedef {import("./templates/DocumentInstance.mjs").DocumentInstance} DocumentInstance */

/** @typedef {import("./types/general.mjs").Metadata} Metadata */
/**
 * @typedef {Object} DocumentRenderSource
 * @property {string} id
 ...
 * @property {Metadata} variables
 * @property {string | null} break_mode
 * @property {string | null} horizontal_rule_behavior
 */
...
export class Document {
    ...
    static parseFileInfo(rel_path) {
        ...
        const record_prefixed_match = base_name.match(
            /^[A-Z]{2,5}-\d{5}_([A-Z]{2,5})(?:-|_|$)/
        );
        const legacy_match = base_name.match(/^([A-Z]{2,5})(?:-|_)/);
        const doc_type = record_prefixed_match
            ? record_prefixed_match[1]
            : legacy_match
            ? legacy_match[1]
            : null;
    ...
    /**
     * Load a resource relative to this document, using the same file loading as load().
     * @param {string} relativePath
     * @returns {Uint8Array}
     */
    loadResource(relativePath) {
        if (!this.source_path)
            throw new Error(
                "A document resource requires a Markdown source path."
            );
        if (
            !relativePath ||
            ...
            throw new Error(`Invalid document resource path: ${relativePath}`);
        }
        const directory = realpathSync(dirname(this.source_path));
        ...
        if (
            ...
            throw new Error("Document resource leaves the Markdown directory.");
        }
        ...
        if (
            ...
            throw new Error(
                "Document resource must be a file within the Markdown directory."
            );
        }
     ...
     * RenderPack configuration remains in snake_case until the renderer boundary.
     * @param {RenderPack} render_pack
     * @param {DocumentRenderEntry} entry
     * @param {string} rel_record_path
     * @param {{ disable_soft_wrap?: boolean; instance?: DocumentInstance }} [options]
     * @returns {DocumentRenderSource}
     */
    toRenderSource(render_pack, entry, rel_record_path, options) {
        ...
        const parsed = parseMarkdownDoc(this.text, {
            softWrap: options?.disable_soft_wrap !== true
        });
        const converter = new MarkdownToAstConverter({
            sourceDocument: this,
            instance: options?.instance
        });
        const ast = converter.convert(parsed);
        options?.instance?.bindSource(ast.root);
