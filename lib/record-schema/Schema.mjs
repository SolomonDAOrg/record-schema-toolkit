...
export class Schema {
     ...
     */
    resolveRef(ref) {
        return Schema._resolveRef(this.definition, ref, this.source_path).schema;
    }
     ...
     * @private
     */
    static _resolveRef(root, ref, source_path) {
        const hashIndex = ref.indexOf("#");
        const filePart = hashIndex >= 0 ? ref.slice(0, hashIndex) : "";
        ...
        if (filePart.length > 0) {
            ...
            const abs = resolve(dirname(source_path), filePart);
            const externalRoot = /** @type {Metadata} */ (readJson(abs));
            const externalSchema = fragment.length > 0
        ...
        return {
            schema: Schema._resolveInternalRef(root, fragment.startsWith("#") ? fragment : `#${fragment}`),
            root,
            source_path
        };
    }
     ...
     * @private
     */
    static _validateWithSchema(v, schema, root, path, errors, source_path) {
        if (!isObject(schema)) {
            return;
        }
        ...
        if (hasPropertyOfType(schema, "$ref", "string")) {
            try {
                const resolved = Schema._resolveRef(root, schema["$ref"], source_path);
                Schema._validateWithSchema(
                    v,
                    resolved.schema,
                    resolved.root,
                ...
                errors.push({
                    path,
                    message: err instanceof Error ? err.message : "Unable to resolve $ref"
                });
            }
            return;
        }
        ...
        if (isArray(schema["allOf"])) {
            ...
            for (let i = 0, len = arr.length; i < len; i++) {
                Schema._validateWithSchema(v, arr[i], root, path, errors, source_path);
            }
        }
        ...
        if (isArray(schema["anyOf"])) {
            ...
            for (let i = 0, len = arr.length; i < len; i++) {
                /** @type {SchemaError[]} */
                const local = [];
                Schema._validateWithSchema(v, arr[i], root, path, local, source_path);
                if (local.length === 0) {
                    return;
                }
        ...
        const t = schema["type"];
        if (typeof t === "string") {
            Schema._validateType(v, t, schema, root, path, errors, source_path);
        }
    }
     ...
     * @private
     */
    static _validateType(v, t, schema, root, path, errors, source_path) {
        if (t === "object") {
            Schema._validateObject(v, schema, root, path, errors, source_path);
            return;
        }

        if (t === "array") {
            Schema._validateArray(v, schema, root, path, errors, source_path);
            return;
        }
     ...
     * @private
     */
    static _validateObject(v, schema, root, path, errors, source_path) {
        if (!isObject(v)) {
     ...
     * @private
     */
    static _validateArray(v, schema, root, path, errors, source_path) {
        if (!isArray(v)) {
