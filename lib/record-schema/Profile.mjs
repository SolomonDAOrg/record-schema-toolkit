...
export class Profile {
    ...
    /**
     * Get pack paths for document policies, optionally filtered by pack type.
     * @param {'formatting' | 'render'} [type]
     * @returns {string[]}
     */
    getPackPaths(type) {
        const references = this.data?.rules?.document_policies?.pack_paths ?? [];
        return references
            .filter(reference => isString(reference)
                ? type === undefined || type === "formatting"
                : type === undefined || reference.type === type)
            .map(reference => isString(reference) ? reference : reference.path);
    }
    ...
    /**
     * Get repository-relative META overlay schema paths.
     * @returns {string[]}
     */
    getMetaOverlaySchemaPaths() {
        return arrayOr(this.data?.rules?.meta_policies?.overlay_schema_paths);
    }

    /**
     * Get structured document schema bindings.
     * @returns {{ doc_type: string, schema_path: string, extensions: string[], required: boolean, min_count: number | null, max_count: number | null }[]}
     */
    getStructuredDocumentSchemas() {
        const bindings = arrayOr(this.data?.rules?.structured_document_schemas);
        /** @type {{ doc_type: string, schema_path: string, extensions: string[], required: boolean, min_count: number | null, max_count: number | null }[]} */
        const out = [];
        for (let i = 0, len = bindings.length; i < len; i++) {
            ...
            const doc_type = stringOr(binding.doc_type, "");
            const schema_path = stringOr(binding.schema_path, "");
            if (doc_type.length === 0 || schema_path.length === 0) {
                continue;
            }
            const extensions = arrayOr(binding.extensions).filter(isString);
            out.push({
                doc_type,
                schema_path,
                extensions,
                ...
                min_count:
                    Number.isSafeInteger(binding.min_count) &&
                    binding.min_count >= 0
                        ? binding.min_count
                        : null,
                max_count:
                    Number.isSafeInteger(binding.max_count) &&
                    binding.max_count >= 1
                        ? binding.max_count
                        : null
            });
    ...
    /**
     * Get the profile extension map.
     * @returns {Record<string, unknown>}
     */
    getExtensions() {
        return isObject(this.data?.extensions)
            ? /** @type {Record<string, unknown>} */ (this.data.extensions)
            : {};
    }

    /**
     * Get one named profile extension without interpreting its shape.
     * @param {string} name
     * @returns {unknown}
     */
    getExtension(name) {
    ...
    /**
     * Enumerate profile extensions in declaration order.
