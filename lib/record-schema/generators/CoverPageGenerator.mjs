...
export class CoverPageGenerator {
    ...
    _resolveVersion(overrides, cover_fmt, meta) {
        return (
            ...
            stringOr(meta?.assembly?.packet?.version) ??
    ...
    _resolveDocumentId(overrides, cover_fmt, record) {
        return (
            ...
            stringOr(record.metafile?.data?.assembly?.packet?.document_id) ??
            stringOr(record.metafile?.data?.document?.document_id) ??
            stringOr(cover_fmt?.document_id) ??
            (record.metafile?.data?.id ||
                record.record_id ||
                "DOC-[YYYY]-[###]")
        );
    }
