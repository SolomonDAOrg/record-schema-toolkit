...
import { createTextVariableRule } from "../../ast/formatting/text-variables.mjs";
import { createHeadingPageBreakRule } from "../../ast/formatting/heading-page-breaks.mjs";
import { resolvePacketVariables } from "../render/packet-variables.mjs";
...
import { resolvePageConfiguration } from "../../ast/adapters/RenderPackAdapter.mjs";
import { Document } from "../Document.mjs";
import { DocumentInstance } from "../templates/DocumentInstance.mjs";
import { loadPdf } from "../../pdf/reader.mjs";
...
export class FilingPacketGenerator {
    ...
    constructor(
        ...
        render_pack,
        verbose,
        provided_render_pack_is_authoritative
    ) {
        /** @type {Repository} */
        this._repo = repo;
        ...
        /** @type {boolean} */
        this._provided_render_pack_is_authoritative =
            provided_render_pack_is_authoritative === true;
    ...
    _extractRenderSelection(metafile_or_data) {
        return Repository.extractRenderSelection(metafile_or_data);
    }
    ...
    _hasExplicitRenderSelection(selection) {
        return !!(
            selection &&
            ((Array.isArray(selection.pack_ids) &&
                selection.pack_ids.length > 0) ||
                (Array.isArray(selection.pack_paths) &&
                    selection.pack_paths.length > 0) ||
    ...
    _renderPackLooksResolved(render_pack, record, meta) {
        ...
        const base_config = render_pack.resolveForFile({
            ...
            ext: "pdf",
            render_profile_id:
                record.metafile?.resolveRenderMetadata().render_profile_id
        });
        ...
        return pack_id !== "default" || has_cover_layout || has_packet_identity;
    }
    ...
    _mergeLoadedRenderPacksForSelection(packs, selection) {
            ...
            const include_pack = (pack) => {
                ...
                if (typeof source_path === "string" && source_path.length > 0) {
                    ...
                    for (let i = 0, len = imports.length; i < len; i++) {
                        const imported_pack = by_source.get(
                            join(source_dir, imports[i])
                        );
                        if (imported_pack) {
        ...
        const select_by_family = (all_packs, family) => {
            ...
            return matched_ids.length > 0
                ? select_by_ids(all_packs, matched_ids)
                : [];
        };
        ...
        const with_support_packs = (all_packs, selected_packs) => {
            ...
            const merged_packs = all_packs.filter((pack) => {
                const normalized_id = normalize_key(pack.getId());
                const descriptor = `${normalized_id} ${basename(
                    pack.source_path || ""
                )} ${pack.getDescription() || ""}`.toLowerCase();
                ...
                return (
                    ...
                    (typeof pack.source_path === "string" &&
                        selected_sources.has(pack.source_path)) ||
                    is_support_pack
                );
            });
    ...
    generate(record, options) {
        const instance = DocumentInstance.load(this._repo, record, options);
        if (instance) {
            if (this._provided_render_pack_is_authoritative)
                throw new Error("Bound templates select render packs through configuration.");
            for (const [key, value] of Object.entries(options)) {
                if (["instance", "stage", "overwrite"].includes(key)) continue;
                if (key === "cover_overrides") {
                    if (Object.values(value ?? {}).some(item => item !== undefined && item !== false))
                        throw new Error("Bound templates take cover values from configuration.");
                } else if (value) throw new Error("Bound templates take this setting from configuration: " + key);
            }
            record = {...record, metafile: instance.metafile};
        }
        ...
        if (!disable_page_breaks && base_config?.heading_page_breaks) {
            pipeline.addFormattingRule(createHeadingPageBreakRule(
                deepSnakeToCamel(base_config.heading_page_breaks)
            ));
        }
        ...
        const packet_variables = resolvePacketVariables(
            record.metafile, normalized_cover_overrides
        );

        if (base_config?.text_variables) {
            pipeline.addFormattingRule(createTextVariableRule(base_config.text_variables, packet_variables));
        }
        ...
        const packet_page_config = resolvePageConfiguration({
            pageSize: base_config?.page_size,
        ...
        const composition_config = {
            ...
            sections: sources.map((src, idx) => ({
                ...
                headers: src.is_image_page
                    ? []
                    : base_config?.default_headers
                    ? deepSnakeToCamel(base_config.default_headers)
                    : [
                          {
        ...
        const effective_status_phase =
            meta_status_phase !== "" ? meta_status_phase : cover_status;
        ...
        this._trace(
            `  draftState: coverStatus=${
                cover_status || "(unset)"
            } metaStatus=${
                meta_status_phase || "(unset)"
            } hasDraftStatus=${has_draft_status} watermarkEnabled=${watermark_enabled} showDraftMarking=${show_draft_marking}`
        );
        ...
        const renderer = createTwoPassPdfRenderer({
            pageConfig: resolvePageConfiguration(renderer_cfg),
            ...
            headingStyles: renderer_cfg.headingStyles,
            pageTracking: renderer_cfg.pageTracking,
            pageComposition: renderer_cfg.pageComposition,
            pageNumbering: renderer_cfg.pageNumbering,
            verbose: this._verbose,
            variables: {
                ...packet_variables,
            ...
            defaultFooters: renderer_cfg.defaultFooters ?? [
                {
                    pages: "all",
                    columns: {
                        ...(show_draft_marking
            ...
            directiveStyles: renderer_cfg.directiveStyles,
            markdownEnvelopeStyle: renderer_cfg.markdownEnvelopeStyle,
            tocConfig: {
                levelStyles
            },
            leadingSection: leading_section_config,
            ...
            metadata: {
                ...
                subject: instance ? "record-schema-instance:" + instance.bindingHash + ";" + instance.documentId + ";" + instance.stage : packet_title_for_header,
        ...
        if (result.success && result.renderResult?.output) {
            ...
            if (instance && existsSync(out_path) && loadPdf(readFileSync(out_path)).getSignatureFields().some(field => field.valueId !== null))
                throw new Error("A signed PDF cannot be overwritten by rendering. Create a new instance revision and output path.");
    ...
    _loadSources(
        ...
        for (let i = 0, len = pack_entries.length; i < len; i++) {
            ...
            if (!doc) {
                throw new Error(`Packet source does not exist: ${entry.path}`);
            }
