...
export class FilingPacketGenerator {
        ...
        this._trace(
            `  merged: ${
                merged_cover_overrides
                    ? JSON.stringify(Object.keys(merged_cover_overrides))
                    : "(null)"
            }`
        );
                ...
                headers: [
                    {
                        ...
                        columns: {
                            ...
                            right: {
                                content: `${packet_title_for_header} - ${this._buildHeaderDocName(
                                    src.name,
                                    entity_name,
                                    pkt_cfg
                                ).toUpperCase()}`,
            ...
            metadata: {
                title:
                    entity_name !== pkt_cfg.default_entity_name
                        ? `${entity_name} ${packet_title_for_header}`
     ...
     * @returns {Array<{ id: string, name: string, root: any, metadata: Metadata, headerTitle: string, variables: Metadata, break_mode: string|null, horizontal_rule_behavior: string|null }>}
     * @private
     */
    _loadSources(record, pack_entries, pkt_cfg, entity_name, base_config) {
        const sources = [];
        ...
        for (let i = 0, len = pack_entries.length; i < len; i++) {
            ...
            const ast = convertMarkdownToDocument(parseMarkdownDoc(content));
