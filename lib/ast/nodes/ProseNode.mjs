...
export class HeadingNode extends ProseNode {
     ...
     * @param {string | BaseNode[] | null} content
     * @param {BaseNodeData & Partial<HeadingData> & { anchorId?: string }} [data]
     */
    constructor(
        level,
        content,
        data = /** @type {Partial<HeadingData> & { anchorId?: string }} */ ({})
    ) {
 ...
 * @property {Uint8Array} [sourceBytes] - Loaded image resource bytes.
 * @property {string} src
 * @property {string} [alt]
...
export class ImageNode extends ProseNode {
    ...
    constructor(src, data = /** @type {BaseNodeData & ImageData} */ ({})) {
        ...
        if (data.sourceBytes !== undefined)
            this.attrs.sourceBytes = data.sourceBytes;
...
/**
 * Notice / callout block node.
 * Maps to the `"notice"` legal AST node type.
 * Inner content is re-parsed from raw markdown and appended as children.
 */
export class NoticeNode extends ProseNode {
    /**
     * @param {{ title?: string } & BaseNodeData} [data]
     */
    constructor(data = {}) {
        super(PROSE_NODE_TYPES.NOTICE, data);
...
/**
 * Inline signature / acknowledgment panel node.
 * Uses the legal node type `"signature-block"` so the PDF renderer can route
 * it through the signature block layout path, while still carrying normal node
 * attributes and child semantics from BaseNode.
 */
export class SignatureBlockNode extends BaseNode {
    /**
     * @param {BaseNodeData & SignatureBlockData} [data]
     */
    constructor(data = {}) {
        super("signature-block", data);
        this.category = NODE_CATEGORIES.LEGAL;

        /** @type {string | undefined} */
        this.fieldName = data.fieldName;
        if (data.fieldName !== undefined) this.attrs.fieldName = data.fieldName;

        /** @type {string | undefined} */
        this.title = data.title;

        /** @type {string | undefined} */
        this.partyLabel = data.partyLabel;

        /** @type {string | undefined} */
        this.bodyText = data.bodyText;

        /** @type {string[] | undefined} */
        this.fields = Array.isArray(data.fields) ? [...data.fields] : undefined;

        /** @type {Record<string, string> | undefined} */
        this.values = data.values ? { ...data.values } : undefined;
        ...
        if (this.partyLabel !== undefined) {
            this.attrs.partyLabel = this.partyLabel;
        }
        if (this.bodyText !== undefined) {
            this.attrs.bodyText = this.bodyText;
        }
        if (this.fields !== undefined) {
            this.attrs.fields = [...this.fields];
        }
        if (this.values !== undefined) {
            this.attrs.values = { ...this.values };
        }
        if (this.variant !== undefined) {
            this.attrs.variant = this.variant;
        }
        if (this.directive !== undefined) {
            this.attrs.directive = this.directive;
        }
    }
    ...
    toJSON() {
        ...
        if (this.partyLabel) {
            obj.partyLabel = this.partyLabel;
        }
        if (this.bodyText) {
            obj.bodyText = this.bodyText;
        }
        if (this.fields) {
