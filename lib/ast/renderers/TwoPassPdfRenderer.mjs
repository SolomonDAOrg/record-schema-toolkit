...
export class TwoPassPdfRenderer {
    ...
    renderDiagonalWatermark(builder, state, watermark) {
        ...
        const gray = isNumber(watermark.gray) ? watermark.gray : 0.9;
    ...
    collectInlineRunsInto(out, node, inherited) {
        ...
        if (node.type === "definition") {
            ...
            if (!inherited.monospace) {
                defText = applyTypographySubstitutions(defText);
            }
    ...
    renderHeading(node, state, sectionId) {
        ...
        const headingStyles = this.config.headingStyles ?? null;

        const levelKey = `h${level}`;
        const configuredHeadingColor = hasProperty(headingStyles, levelKey)
            ? stringOr(headingStyles[levelKey].color, undefined)
            : undefined;
    ...
    renderNotice(node, state, sectionId, layout) {
        const noticeStyle = this.config.noticeStyle ?? null;

        const nodeBoxStyle = node.boxStyle ?? null;

        const paddingSpec =
            nodeBoxStyle?.padding ??
            (isObject(noticeStyle) ? numberOr(noticeStyle.padding, 10) : 10);
        const pad = this._normalizeBoxPadding(paddingSpec, 10);
        const boxPadX = Math.max(pad.left, pad.right);
        const boxPadY = Math.max(pad.top, pad.bottom);
