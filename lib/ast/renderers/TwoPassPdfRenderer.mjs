...
import {
    PdfDocumentBuilder,
    measureTextWidth,
    formFieldTextBaseline
} from "../../pdf/document.mjs";
import { PdfContentStreamBuilder } from "../../pdf/content-stream.mjs";
import { layoutPageTracking } from "../../pdf/page-tracking/layout.mjs";
import { renderPageTracking } from "../../pdf/page-tracking/render.mjs";
...
import { processDustCover, renderSvgToPdf } from "./DustCoverRenderer.mjs";
import {
    GoogleFontFetcher,
    normaliseFontFamily,
    parseFontStyle,
...
/** @typedef {{ ascent: number; descent: number }} TextInkMetrics */
 ...
 * @typedef {import("../types/core.mjs").FormFieldsStyle} FormFieldsStyle
 * @typedef {import("../types/core.mjs").MarkdownEnvelopeStyle} MarkdownEnvelopeStyle
 * @typedef {import("../nodes/BaseNode.mjs").BaseNode} BaseNode
 * @typedef {import("../nodes/LegalNode.mjs").DefinitionNode} DefinitionNode
 * @typedef {import("../nodes/ProseNode.mjs").ImageData} ImageData
 ...
 * @typedef {import("../types/core.mjs").PageCompositionRegion} PageCompositionRegion
 * @typedef {import("../types/core.mjs").PageCompositionSource} PageCompositionSource
 * @typedef {import("../../pdf/page-tracking/types.mjs").PageTrackingConfig} PageTrackingConfig
 * @typedef {import("../types/core.mjs").LetterheadLogo} LetterheadLogo
 * @typedef {import("../../pdf/document.mjs").FormFieldAnnotation} FormFieldAnnotation
 */

/**
 * @typedef {Object} TableColumnMeasurement
 * @property {PdfBuildState} state
 * @property {number} fontSize
 * @property {Padding} bodyPadding
 * @property {Padding} headerPadding
 ...
 * @property {PageComposition} [pageComposition]
 * @property {{includeSigningPage?: boolean}} [pageNumbering] - Include the final execution page in numbering and render its footer when enabled.
 ...
 * @property {PageTrackingConfig} [pageTracking]
 * @property {Record<string, TextStyle>} [headingStyles]
 * @property {MarkdownEnvelopeStyle} [markdownEnvelopeStyle]
 * @property {BoxStyle} [noticeStyle]
 * @property {BoxStyle} [signatureBlockStyle]
 * @property {Record<string, BoxStyle & FormFieldsStyle & { headerBackgroundColor?: string | number; titleColor?: string; titleBackgroundColor?: string | number; fieldLabelWidth?: number; signatureRowHeight?: number; rowHeight?: number; rowGap?: number; bodyGap?: number; headerGap?: number; postGap?: number; titleFontScale?: number }>} [directiveStyles]
 ...
 * @property {SigningPageConfig} [signingPage] - Final signing/execution page (excluded from pagination by default)
 ...
 * @property {boolean} [ownsPageFurniture] - Positioned regions supply this page's header and footer.
 ...
 * @property {Array<LinkAnnotation | FormFieldAnnotation>} linkAnnotations
 */
...
/**
 * @typedef {Object} MarkdownEnvelopeEntry
...
export class TwoPassPdfRenderer {
    ...
    constructor(config) {
        ...
        /** @type {boolean} */
        this._renderingPageComposition = false;
     ...
     * @returns {Uint8Array}
     */
    renderPageComposition(sections, configuration) {
        const state = this.initializeState();
        this.renderPageCompositionPage(sections, configuration, state);
        this.renumberPages(state);
        this.finalizeInternalLinks(state);
        return this.buildPdf(state);
    }

    /** @param {ReadonlyArray<ComposedSection>} sections @param {PageComposition} configuration @param {PdfBuildState} state @returns {void} */
    renderPageCompositionPage(sections, configuration, state) {
        configuration = this.layoutEngine.layoutLetterheadLogo(
            configuration,
            state.pageWidth,
            state.pageHeight
        );
        const sources = this.layoutEngine.layoutPageComposition(
            configuration,
            sections,
            state.pageWidth,
            state.pageHeight
        );
        const page = this.newPage(state, sections[0].id);
        page.ownsPageFurniture = true;
        for (const decoration of configuration.decorations ?? []) {
            this.drawFilledRect(
                page.contentBuilder,
                decoration.x,
                state.pageHeight - decoration.top - decoration.height,
                decoration.width,
                decoration.height,
                decoration.fill
            );
        ...
        if (
            configuration.letterheadLogo?.enabled !== false &&
            configuration.letterheadLogo
        ) {
            this.renderLetterheadLogo(configuration.letterheadLogo, state);
        }
        ...
        for (const region of configuration.regions) {
            this.renderPageCompositionRegion(
                sources.get(region.sourceHeading),
                region,
                state,
                page
            );
    ...
    /** @param {LetterheadLogo} logo @param {PdfBuildState} state @returns {void} */
    renderLetterheadLogo(logo, state) {
        if (extname(logo.path).toLowerCase() !== ".svg")
            throw new Error("Letterhead logo must be an SVG file");
        const artwork = processDustCover(readFileSync(logo.path, "utf8"), {
            cacheDir: null
        });
        if (
            ![
            ...
            throw new Error(
                "Letterhead logo requires a positive SVG viewBox or intrinsic dimensions"
            );
        }
        const width = logo.width * (logo.scale ?? 1);
        const height = logo.height * (logo.scale ?? 1);
        const fit = Math.min(
            width / artwork.svgWidth,
            height / artwork.svgHeight
        );
        const drawWidth = artwork.svgWidth * fit;
        const drawHeight = artwork.svgHeight * fit;
        renderSvgToPdf(artwork, state.currentPage.contentBuilder, state, {
            x: logo.x + (width - drawWidth) / 2,
            top: logo.top + (height - drawHeight) / 2,
            width: drawWidth,
            height: drawHeight
        });
    }

    /** @param {PageCompositionSource} source @param {PageCompositionRegion} region @param {PdfBuildState} state @param {PdfPage} page @returns {void} */
    renderPageCompositionRegion(source, region, state, page) {
        const bottom = state.pageHeight - region.top - region.height;
        state.margins = {
            left: region.x,
            right: state.pageWidth - region.x - region.width,
            top: region.top,
            bottom: bottom - region.fontSize * (region.lineSpacing ?? 1.3) * 2
        };
        ...
        if (region.showHeading === true)
            this.renderPageCompositionHeading(source.heading, region, state);
        state.currentY -= region.fontSize;
        for (let index = 0; index < source.nodes.length; index++) {
            const node = source.nodes[index];
            switch (node.type) {
                case "image":
                    this.renderPageCompositionImage(node, region, state, page);
                    break;
                case "table":
                    this.renderPageCompositionTable(
                        /** @type {TableNode} */ (node),
                        region,
                        state,
                        page
                ...
                case "signature-block":
                    this.renderPageCompositionSignature(
                        node,
                        region,
                        state,
                        page
                    ...
                    break;
                case "paragraph":
                    this.renderPageCompositionParagraph(
                        node,
                        region,
                        state,
                        page,
                        index + 1 < source.nodes.length
                    );
                    break;
                default:
                    throw new Error(
                        "Unsupported page composition node: " + node.type
                    );
            }
        }
    }

    /** @param {string} text @param {PageCompositionRegion} region @param {PdfBuildState} state @returns {void} */
    renderPageCompositionHeading(text, region, state) {
        const size = region.headingFontSize ?? 8;
        const font = this.getFont(state, true);
        const heading = layoutPlainText(
            text,
            region.width,
            font,
            size,
        ...
        for (const line of heading.lines) {
            this.renderTextAt(
                state.currentPage.contentBuilder,
                state,
                line,
                anchorX,
                state.currentY,
                ...
                font,
                align,
                region.headingColor ?? region.color
            );
            state.currentY -= size * 1.2;
        }
        state.currentY += size * 1.2 - (region.headingGap ?? 7);
    }

    /** @param {PageCompositionRegion} region @param {PdfBuildState} state @param {PdfPage} page @param {number} inkBottom @returns {void} */
    assertPageCompositionFits(region, state, page, inkBottom) {
        if (
            state.currentPage !== page ||
            inkBottom < state.pageHeight - region.top - region.height
        ) {
            throw new Error(
                "Page composition overflow in source heading: " +
                    region.sourceHeading
            );
        }
    }

    /** @param {BaseNode} node @param {PageCompositionRegion} region @param {PdfBuildState} state @param {PdfPage} page @returns {void} */
    renderPageCompositionImage(node, region, state, page) {
        state.currentY += region.fontSize;
        const width = Math.min(numberOr(node.attrs.width, 200), region.width);
        const height = Math.min(
            numberOr(node.attrs.height, 150),
            region.height
        );
        this.assertPageCompositionFits(
            region,
            state,
            page,
        ...
        this.renderImage(
            { ...node, attrs: { ...node.attrs, width, height } },
            state
        );
        this.assertPageCompositionFits(
            region,
            state,
            page,
            state.lastInkBottomY
        );
        state.currentY =
            state.lastInkBottomY - region.fontSize - (region.paragraphGap ?? 4);
    }

    /** @param {TableNode} node @param {PageCompositionRegion} region @param {PdfBuildState} state @param {PdfPage} page @returns {void} */
    renderPageCompositionTable(node, region, state, page) {
        state.currentY += region.fontSize;
        const paragraphBottom = state.margins.bottom;
        state.margins.bottom = state.pageHeight - region.top - region.height;
        this.renderTable(node, state, page.sectionId);
        this.assertPageCompositionFits(
            region,
            state,
            page,
        ...
        state.margins.bottom = paragraphBottom;
        state.currentY =
            state.lastInkBottomY - region.fontSize - (region.paragraphGap ?? 4);
    }

    /** @param {BaseNode} node @param {PageCompositionRegion} region @param {PdfBuildState} state @param {PdfPage} page @returns {void} */
    renderPageCompositionSignature(node, region, state, page) {
        state.currentY += region.fontSize;
        this.assertPageCompositionFits(
            region,
            state,
            page,
            state.currentY - this._estimateSignatureBlockHeight(node, state)
        );
        this.renderSignatureBlock(node, state);
        this.assertPageCompositionFits(region, state, page, state.currentY);
        state.currentY -= region.fontSize;
    }

    /** @param {BaseNode} node @param {PageCompositionRegion} region @param {PdfBuildState} state @param {PdfPage} page @param {boolean} hasFollowingNode @returns {void} */
    renderPageCompositionParagraph(
        node,
        region,
        state,
        page,
        ...
        const runs = this.buildInlineRuns(node).map((run) => ({
            ...run,
            bold: region.bold ?? run.bold,
            color: run.color ?? region.color
        }));
        const lineSpacing = region.lineSpacing ?? 1.3;
        const align = region.align ?? "left";
        if (align !== "left") {
            const text = runs.map((run) => run.text).join("");
            const font = this.getFont(state, region.bold ?? false);
            const layout = layoutPlainText(
                text,
                region.width,
                font,
                region.fontSize,
                region.fontSize * lineSpacing
            );
            const anchorX =
                align === "right"
                    ? region.x + region.width
            ...
            for (const line of layout.lines) {
                ...
                state.currentY -= region.fontSize * lineSpacing;
            }
        } else {
        ...
        this.assertPageCompositionFits(
            ...
            state.currentY +
                region.fontSize * lineSpacing -
                region.fontSize * 0.35
        );
        if (hasFollowingNode) state.currentY -= region.paragraphGap ?? 4;
    }
    ...
    render(sections, coverPage, toc, dustCoverPage) {
        ...
        try {
            if (this.config.pageComposition?.mode === "fixed") {
                if (
                    coverPage ||
                    toc ||
                    resolvedDustCoverPage ||
                    this.config.signingPage ||
                    this.config.defaultHeaders?.length ||
                    this.config.defaultFooters?.length ||
                    sections.some(
                        (section) =>
                            section.config.headers?.length ||
                            section.config.footers?.length
                    )
                ) {
                    throw new Error(
                        "Fixed page composition owns its complete page furniture"
                    );
                }
                const output = this.renderPageComposition(
                    sections,
                    this.config.pageComposition
                );
                return {
                    success: true,
                    output,
            ...
            const firstPageComposition = this.config.pageComposition
                ? this.layoutEngine.partitionPageComposition(
                      this.config.pageComposition,
                      sections
                  )
                : null;
            if (
                firstPageComposition &&
                (coverPage || toc || resolvedDustCoverPage)
            ) {
                throw new Error(
                    "First-page composition cannot be combined with a separate cover or table of contents."
                );
            }
            const flowingSections = firstPageComposition
                ? [firstPageComposition.continuation]
                : sections;
            ...
            // =================================================================
            const normalizedSections = this.normalizeSections(flowingSections);
            const signingSections = [];
            for (const section of normalizedSections) {
                const body = [];
                for (const node of section.content) {
                    if (node.attrs?.directive === "signing-page") {
                        signingSections.push({
                            ...section,
                            id: "signing",
                            config: {
                                ...section.config,
            ...
            if (
                signingSections.length > 1 ||
                (signingSections.length &&
                    this.config.signingPage?.enabled !== false &&
                    this.config.signingPage?.parties?.length)
            ) {
                throw new Error(
                    "A document must have one signing page source: either a signing-page directive or signingPage configuration."
                );
            }
            ...
            if (firstPageComposition) {
                this._renderingPageComposition = true;
                this.renderPageCompositionPage(
                    [firstPageComposition.firstPage],
                    {
                        mode: "fixed",
                        pagination: this.config.pageComposition.pagination,
                        placeholderMode:
                            this.config.pageComposition.placeholderMode,
                        letterheadLogo:
                            this.config.pageComposition.letterheadLogo,
                        regions: this.config.pageComposition.regions,
                        decorations: this.config.pageComposition.decorations
                    },
                    state
                );
                this._renderingPageComposition = false;
            ...
            for (let i = 0, len = normalizedSections.length; i < len; i++) {
                if (normalizedSections[i].content.length === 0) continue;
            ...
            for (const section of signingSections) {
                const before = state.pages.length;
                state.lastNodeType = null;
                this.renderSection(section, state, layoutResult);
                if (
                    state.pages.length !== before + 1 ||
                    state.currentY < state.margins.bottom
                ) {
                    throw new Error(
                        "Signing page content must fit on one final page."
                    );
                }
            }

            // Render the final signing/execution page.
            if (
                this.config.signingPage &&
                this.config.signingPage.enabled !== false &&
                ...
                const beforeSigning = state.pages.length;
                this.renderSigningPage(this.config.signingPage, state);
                if (
                    state.pages.length !== beforeSigning + 1 ||
                    state.currentY < state.margins.bottom
                ) {
                    throw new Error(
                        "Signing page content must fit on one final page."
    ...
    initializeState() {
        ...
        const pageWidth =
            pageConfig.width ?? (landscape ? size.height : size.width);
        const pageHeight =
            pageConfig.height ?? (landscape ? size.width : size.height);
    ...
    renumberPages(state) {
        ...
        for (let i = 0, len = state.pages.length; i < len; i++) {
            ...
            if (
                sectionId === "signing" &&
                this.config.pageNumbering?.includeSigningPage !== true
            ) {
                continue;
            }
    ...
    getVerticalSpacing(node, previousNodeType) {
        ...
        // A list straight after an item's own line is that item's nested list: it continues
        // the item, so no gap opens above it whatever the pack's list spacing is.
        if (node.type === "list" && previousNodeType === "list-item") {
            return 0;
        }
    ...
    /**
     * Calculate MAX offset across the header/footer configs that apply to the current page.
     ...
     * @returns {number}
     */
    calculateMaxHeaderFooterOffset(
        configs,
        pageNumber,
        sectionPageNumber,
        totalPages,
        ...
        for (let i = 0, len = configs.length; i < len; i++) {
            ...
            const offset = this.calculateHeaderFooterOffset(config);
            if (offset > maxOffset) {
                maxOffset = offset;
            }
        }
    ...
    newPage(state, sectionId) {
        ...
        const headerMaxOffset = this.calculateMaxHeaderFooterOffset(
            headers,
            pageNumber,
            sectionPageNumber,
            pageNumber,
        ...
        const footerMaxOffset = this.calculateMaxHeaderFooterOffset(
            footers,
            pageNumber,
            sectionPageNumber,
            pageNumber,
     ...
     * @param {PdfBuildState} state
     * @param {LinkAnnotation | import("../../pdf/document.mjs").FormFieldAnnotation} annotation
     * @returns {void}
     */
     ...
     * @param {PdfBuildState} state
     * @param {import("../../pdf/document.mjs").FormFieldAnnotation} annotation
     * @returns {void}
     */
    addFormFieldAnnotation(state, annotation) {
        this.addPageAnnotation(state, {
            ...annotation,
            fontName: annotation.fontName ?? this.getFont(state, false)
        });
    }
    ...
    /**
     * @param {PdfBuildState} state @param {string} text
     * @param {string} baseFont @param {number} fontSize
     * @returns {TextInkMetrics}
     */
    measureTextInk(state, text, baseFont, fontSize) {
        const metrics = state.doc.embeddedFonts.get(
            this.getFontResourceName(state, baseFont)
        )?.metrics;
        ...
        if (metrics) {
            for (const character of text) {
                ...
                const bounds = metrics.glyphBoundsForCodePoint(
                    character.codePointAt(0)
                );
                if (bounds) {
                    minimum = Math.min(minimum, bounds.yMin);
                    maximum = Math.max(maximum, bounds.yMax);
                }
            }
            if (Number.isFinite(minimum))
                return {
                    ascent: (maximum / metrics.unitsPerEm) * fontSize,
                    descent: (-minimum / metrics.unitsPerEm) * fontSize
                };
        }
        return {
            ascent:
                fontSize *
                (metrics ? metrics.capHeight / metrics.unitsPerEm : 0.7),
            descent: this._textHasDeepDescenders(text)
                ? fontSize *
                  (metrics ? -metrics.descender / metrics.unitsPerEm : 0.2)
                : 0
        };
    }

    /** First-line ascent for nodes whose cursor is a text baseline.
     * @param {BaseNode} node @param {PdfBuildState} state @returns {number}
     */
    getFlowNodeAscent(node, state) {
        const baseFontSize = this.config.baseFontSize ?? 10;
        if (node.type === "heading") {
            const level =
                /** @type {any} */ (node).level ?? node.attrs?.level ?? 1;
            const style = this.getHeadingLevelAliasedValue(
                this.config.headingStyles,
                level
            );
            const font = this.getSemanticStyledFont(
                state,
                this.getHeadingSemanticRole(level),
                this.getFont(state, style?.bold ?? true),
                { bold: style?.bold ?? true }
            );
            const size = baseFontSize * this.getHeadingScale(level);
            const text = applyTypographySubstitutions(this.extractText(node));
            const line =
                this.wrapText(text, state.contentWidth, font, size)[0] ?? "";
            return this.measureTextInk(state, line, font, size).ascent;
        }
        if (node.type !== "paragraph") return 0;
        let width = 0,
            ascent = 0;
        for (const run of this.buildInlineRuns(node)) {
            ...
            for (const token of this.tokenizeInlineText(run.text)) {
                if (token.kind === "newline") return ascent;
                const tokenWidth = measureTextWidth(
                    token.text,
                    font,
                    baseFontSize
                );
                if (width > 0 && width + tokenWidth > state.contentWidth)
                    return ascent;
                width += tokenWidth;
                if (token.kind === "text")
                    ascent = Math.max(
     ...
     * @param {BaseNode} node @param {PdfBuildState} state @returns {number}
     */
    getFlowNodeY(node, state) {
        const bottom = this._getLastInkBottomY(state);
        if (
            this.config.spacingPolicy?.basis !== "ink" ||
            bottom === null ||
            state.lastNodeType === null
        )
        ...
        return (
            bottom -
            this.getVerticalSpacing(node, state.lastNodeType) -
            this.getFlowNodeAscent(node, state)
        );
    }
     ...
     * @returns {void}
     */
    _recordInkFromTextBaseline(state, baselineY, fontSize, text, baseFont) {
        ...
        const descent =
            this.config.spacingPolicy?.basis === "ink"
                ? this.measureTextInk(
                      state,
                      text,
                      baseFont ?? this.getFont(state, false),
                      fontSize
                  ).descent
                : this._estimateTextDescentPt(text, fontSize);
    ...
    renderTextAt(
        ...
        if (this._isContentBuilder(state, builder)) {
            this._recordInkFromTextBaseline(
                state,
                y,
                fontSize,
                textToUse,
    ...
    /**
     * Measure from the first heading baseline to its ink bottom or trailing
     * cursor position, according to the spacing policy.
     *
     * @private
     * @param {BaseNode} node
     * @param {PdfBuildState} state
    ...
    _estimateRenderedHeadingHeight(node, state) {
        ...
        const align = this.resolveNodeHorizontalAlign(
            node,
            configuredHeadingStyle?.align ?? (level === 1 ? "center" : "left")
        );
        const headingSemanticRole = this.getHeadingSemanticRole(level);
        const baseFont = this.getSemanticStyledFont(
            state,
            headingSemanticRole,
            this.getFont(state, configuredHeadingStyle?.bold ?? true),
            { bold: configuredHeadingStyle?.bold ?? true }
        );
        ...
        if (this.config.spacingPolicy?.basis === "ink") {
            return (
    ...
    preflightHeadingFollowerBreak(
        ...
        const inkSpacing = this.config.spacingPolicy?.basis === "ink";
        const remainingBefore =
            (inkSpacing
                ? this.getFlowNodeY(headingNode, state)
                : state.currentY) - state.margins.bottom;
        const remainingAfterHeading =
            ...
            nextSpacing -
            (inkSpacing
                ? this.getFlowNodeAscent(nextNode, state)
        ...
        if (nextNode.type === "table") {
            const openingHeight = this.measureTableOpeningHeight(
                /** @type {TableNode} */ (nextNode),
                state
            );
            if (
                openingHeight > remainingAfterHeading &&
                openingHeight <= state.contentHeight
            )
                this.newPage(state, sectionId);
            return;
        }
    ...
    renderSection(section, state, layout) {
        ...
        /** @type {MarkdownEnvelopeEntry[]} */
        const envelope = [];
        const envelopeStyle = this.config.markdownEnvelopeStyle;
        const envelopeLabels = new Set(
            (
                envelopeStyle?.labels ?? [
                    "Status",
                    "Owner",
                    "Applies to",
                    "Related"
        ...
        for (const node of section.content) {
            if (node.type === "heading" && envelope.length === 0) continue;
            const label =
                node.type === "paragraph"
                    ? this.extractText(node.children[0] ?? node).trim()
                    : "";
            if (!envelopeLabels.has(label.toLowerCase())) break;
            envelope.push({ node, label });
        }
        const styledEnvelope =
            envelope.length > 0 &&
            envelopeStyle &&
            envelopeStyle.enabled !== false
                ? this.measureMarkdownEnvelope(envelope, envelopeStyle, state)
                : null;
        const envelopeStart = styledEnvelope
            ? section.content.indexOf(envelope[0].node)
            : -1;
        let envelopeEnd = envelopeStart + envelope.length;
        const delimiter = section.content[envelopeEnd];
        const delimiterBehavior =
            delimiter?.attrs.hrBehaviorOverride ??
            delimiter?.attrs.hr_behavior_override ??
            state.sectionConfigs.get(section.id)?.horizontalRuleBehavior ??
            this.config.horizontalRule?.behavior ??
            "rule";
        if (
            styledEnvelope &&
            delimiter?.type === "horizontal-rule" &&
            delimiterBehavior !== "page-break"
        ) {
            envelopeEnd++;
        }
        if (styledEnvelope) {
            const openingHeight = this.measureMarkdownEnvelopeFollower(
                section.content,
                envelopeEnd,
                state
            );
            const titleHeight = section.content
                .slice(0, envelopeStart)
                .reduce(
                    (height, node) =>
                        height +
                        this.getVerticalSpacing(node, "heading") +
                        this._estimateRenderedHeadingHeight(node, state),
                    0
                );
            const requiredHeight =
                titleHeight +
                styledEnvelope.spacingBefore +
                styledEnvelope.height +
                styledEnvelope.clearanceAfter +
                openingHeight;
            if (
                requiredHeight <= state.contentHeight &&
                requiredHeight > state.currentY - state.margins.bottom
            ) {
                this.newPage(state, section.id);
            }
        } else if (envelope.length > 1) {
            const labelWidth =
                Math.max(
                    ...envelope.map(({ label }) =>
                        measureTextWidth(
                            label,
                            this.getFont(state, true),
                            this.config.baseFontSize ?? 10
                        )
                    )
                ) + 12;
            for (const { node, label } of envelope) {
                node.attrs.envelopeLabel = label;
                node.attrs.envelopeLabelWidth = labelWidth;
            }
        }

        // Render section content
        for (let i = 0, len = section.content.length; i < len; i++) {
            if (styledEnvelope && i === envelopeStart) {
                this.renderMarkdownEnvelope(
                    styledEnvelope,
                    state,
                    section.id,
                    this.measureMarkdownEnvelopeFollower(
                        section.content,
                        envelopeEnd,
                        state
                    ),
                    section.content[envelopeEnd]
                );
                i = envelopeEnd - 1;
                continue;
            }
    ...
    renderNode(node, state, sectionId, layout, depth) {
        ...
        const inkSpacing =
            this.config.spacingPolicy?.basis === "ink" &&
            ["heading", "paragraph", "table", "signature-block"].includes(
                node.type
            );
        if (inkSpacing) state.currentY = this.getFlowNodeY(node, state);
        ...
        if (
            ...
            if (
                (inkSpacing && totalLines - overflowLines < 2) ||
                overflowLines <= WIDOW_LINE_THRESHOLD ||
        ...
        switch (node.type) {
            ...
            case "section":
                if (node.attrs.directive === "form-fields") {
                    this.renderFormFields(node, state, sectionId);
                } else {
            ...
            case "form-field":
                this.renderTextArea(node, state, sectionId);
                break;
    ...
    /**
     * @param {MarkdownEnvelopeEntry[]} envelope
     * @param {MarkdownEnvelopeStyle} style
     * @param {PdfBuildState} state
     */
    measureMarkdownEnvelope(envelope, style, state) {
        const fontSize = style.fontSize ?? this.config.baseFontSize ?? 10;
        const labelFontSize = style.labelFontSize ?? fontSize * 0.9;
        const lineSpacing = style.lineSpacing ?? 1.35;
        const rowGap = style.rowGap ?? 3;
        const padding = this._normalizeBoxPadding(style.padding, 10);
        const labelFont = this.getFont(state, true);
        const labelWidth =
            Math.max(
                ...envelope.map((entry) =>
                    measureTextWidth(entry.label, labelFont, labelFontSize)
                )
            ) + (style.labelGap ?? 14);
        const valueWidth =
            state.contentWidth - padding.left - padding.right - labelWidth;
        if (valueWidth < fontSize * 8)
            throw new Error(
                "Markdown envelope labels leave insufficient width for values."
            );
        const rows = envelope.map(({ node, label }) => {
            const runs = node.children
                .slice(1)
                .flatMap((child) => this.buildInlineRuns(child))
                .map((run) => ({
                    ...run,
                    color: run.color ?? style.valueColor
                }));
            ...
            return {
                label,
                runs,
                height: Math.max(lines * fontSize, labelFontSize) * lineSpacing
            };
        });
        const height =
            padding.top +
        ...
        const spacingBefore = style.spacingBefore ?? 8;
        const spacingAfter = style.spacingAfter ?? 14;
        // The following text starts at a baseline. Reserve an em for that first
        // line in addition to the requested clear gap below the metadata box.
        const clearanceAfter = spacingAfter + (this.config.baseFontSize ?? 10);
        if (height + clearanceAfter > state.contentHeight)
            throw new Error("A Markdown envelope must fit on one page.");
        return {
            style,
            fontSize,
            labelFontSize,
    ...
    /**
     * @param {ReadonlyArray<BaseNode>} content
     * @param {number} start
     * @param {PdfBuildState} state
     * @returns {number}
     */
    measureMarkdownEnvelopeFollower(content, start, state) {
        let height = 0;
        /** @type {NodeType | null} */
        ...
        for (let index = start; index < content.length; index++) {
            ...
            if (
                ...
                break;
            height += this.getVerticalSpacing(node, previousType);
            if (node.type === "heading") {
                height += this._estimateRenderedHeadingHeight(node, state);
                previousType = node.type;
                continue;
            }
            const nodeHeight = this.estimateNodeHeight(node, state, 0);
            const lineHeight =
                (this.config.baseFontSize ?? 10) *
                (this.config.lineHeight ?? 1.5);
            height +=
                node.keepRules?.keepTogether || nodeHeight <= 4 * lineHeight
                    ? Math.min(nodeHeight, state.contentHeight)
                    : 2 * lineHeight;
            break;
        }
        return height;
    ...
    /**
     * @param {ReturnType<TwoPassPdfRenderer["measureMarkdownEnvelope"]>} envelope
     * @param {PdfBuildState} state
     * @param {string} sectionId
     * @param {number} followerHeight
     * @param {BaseNode} [follower]
     * @returns {void}
     */
    renderMarkdownEnvelope(
        ...
        const inkSpacing = this.config.spacingPolicy?.basis === "ink";
        const inkBottom = this._getLastInkBottomY(state);
        if (inkSpacing && inkBottom !== null) state.currentY = inkBottom;
        const atTop = inkSpacing
            ? inkBottom === null
            : state.currentY >= state.pageHeight - state.margins.top - 0.001;
        const leadingSpace = atTop ? 0 : spacingBefore;
        const groupHeight = height + clearanceAfter;
        const requiredHeight =
            groupHeight + followerHeight <= state.contentHeight
                ? groupHeight + followerHeight
                : groupHeight;
        if (
            requiredHeight + leadingSpace >
            state.currentY - state.margins.bottom
        ) {
            this.newPage(state, sectionId);
        } else {
            state.currentY -= leadingSpace;
        }
        const top = state.currentY;
        const bottom = top - height;
        const builder = state.currentPage.contentBuilder;
        const left = state.margins.left;
        if (
            style.backgroundColor !== undefined &&
            !this._isColorNone(style.backgroundColor)
        ) {
            this.drawFilledRect(
                builder,
                left,
        ...
        if ((style.topRule?.width ?? 0) > 0) {
            this.drawLine(
                builder,
                left,
                top,
                left + state.contentWidth,
                style.topRule.width,
                style.topRule.color
            );
        }
        if ((style.bottomRule?.width ?? 0) > 0) {
            this.drawLine(
                builder,
                left,
                bottom,
                left + state.contentWidth,
                style.bottomRule.width,
                style.bottomRule.color
            );
        }
        let rowTop = top - padding.top;
        for (const row of rows) {
            const baseline = rowTop - Math.max(fontSize, labelFontSize);
            this.renderTextAt(
                builder,
                state,
                row.label,
            ...
            this.renderInlineRunsWrapped(
                row.runs,
                state,
                sectionId,
                left + padding.left + labelWidth,
                baseline,
                valueWidth,
                fontSize,
                lineSpacing
        ...
        this._recordInkBottomY(state, bottom);
        state.currentY =
            bottom -
            (inkSpacing && follower
                ? spacingAfter + this.getFlowNodeAscent(follower, state)
                : clearanceAfter);
        state.lastNodeType = null;
    }
     ...
     * @param {BaseNode} node
     * @param {PdfBuildState} state
     * @returns {{ style: FormFieldsStyle; fontSize: number; rowHeight: number; padding: Padding; labelWidth: number; height: number }}
     */
    measureFormFields(node, state) {
        const style = this.config.directiveStyles?.["form-fields"] ?? {};
        const fontSize = style.fontSize ?? this.config.baseFontSize ?? 10;
        const rowHeight = Math.max(
            style.rowHeight ?? fontSize * 2.2,
            fontSize * 1.5
        );
        const padding = this._normalizeBoxPadding(style.padding, 8);
        const labelWidth = Math.max(
            style.fieldLabelWidth ?? 0,
            ...node.children.map(
                (row) =>
                    measureTextWidth(
                        this.extractText(row.children[0]).trim(),
                        this.getFont(state, true),
                        fontSize
                    ) + 14
            )
        ...
        if (
            ...
            throw new Error(
                "Form-fields labels leave insufficient width for the fields."
            );
        }
        return {
            style,
            ...
            height:
                padding.top +
                padding.bottom +
                rowHeight * node.children.length +
                fontSize
        };
    }
     ...
     * @returns {void}
     */
    renderFormFields(node, state, sectionId) {
        const { style, fontSize, rowHeight, padding, labelWidth, height } =
            this.measureFormFields(node, state);
        if (height > state.contentHeight)
            throw new Error("A form-fields group must fit on one page.");
        if (state.currentY - height < state.margins.bottom)
            this.newPage(state, sectionId);
        const builder = state.currentPage.contentBuilder;
        const top = state.currentY;
        const boxHeight = height - fontSize;
        const bottom = top - boxHeight;
        const labelX = state.margins.left + padding.left;
        const fieldX = labelX + labelWidth;
        const fieldWidth =
            state.contentWidth - padding.left - padding.right - labelWidth;
        if (
            style.backgroundColor !== undefined &&
        ...
        if (
            ...
            !this._isColorNone(style.borderColor)
        ) {
            this._drawStrokedRectColor(
                builder,
                state.margins.left,
        ...
        for (let index = 0; index < node.children.length; index++) {
            ...
            const baseline =
                top - padding.top - (index + 0.5) * rowHeight - fontSize * 0.25;
            this.renderTextAt(
                builder,
                state,
                this.extractText(row.children[0]).trim(),
        ...
        this._recordInkBottomY(state, bottom);
        state.currentY = top - height;
    }

    /**
     * @param {BaseNode} node
     * @param {PdfBuildState} state
     * @returns {{ prompt: string; lines: string[]; boxHeight: number; height: number }}
     */
    measureTextArea(node, state) {
        const fontSize = this.config.baseFontSize ?? 10;
        const step = fontSize * (this.config.lineHeight ?? 1.5);
        const prompt = node.getTextContent() ?? "";
        const lines = prompt
            ? this.wrapText(
                  prompt,
                  state.contentWidth,
            ...
            : [];
        const boxHeight = Math.max(1, numberOr(node.attrs.rows, 4)) * step + 12;
        return {
            prompt,
            lines,
            boxHeight,
            height: lines.length * step + 6 + boxHeight + 16
        };
    }
     ...
     * @returns {void}
     */
    renderTextArea(node, state, sectionId) {
        const field =
            /** @type {import("../nodes/BaseNode.mjs").FormFieldNode} */ (node);
        const metrics = this.measureTextArea(node, state);
        if (metrics.height > state.contentHeight)
            throw new Error("Text area must fit on one page.");
        if (state.currentY - metrics.height < state.margins.bottom)
            this.newPage(state, sectionId);
        const builder = state.currentPage.contentBuilder;
        const fontSize = this.config.baseFontSize ?? 10;
        const step = fontSize * (this.config.lineHeight ?? 1.5);
        ...
        state.currentY -= 6;
        const bottom = state.currentY - metrics.boxHeight;
        this._drawStrokedRectColor(
            builder,
            state.margins.left,
            bottom,
            ...
            metrics.boxHeight,
            0.5,
            this.config.table?.outerBorderColor ?? "#61727E"
        );
        this.addFormFieldAnnotation(state, {
            type: "form",
            fieldType: "text",
            name: field.name ?? `text_area.${++state.formFieldCounter}`,
            tooltip: field.tooltip ?? metrics.prompt,
            multiline: true,
            value: field.value ?? "",
            x: state.margins.left + 4,
            y: bottom + 4,
            width: state.contentWidth - 8,
            height: metrics.boxHeight - 8,
            fontSize
        });
        this._recordInkBottomY(state, bottom);
        state.currentY = bottom - 16;
    }
    ...
    renderParagraph(node, state, sectionId, layout) {
        if (
            isString(node.attrs.envelopeLabel) &&
            isNumber(node.attrs.envelopeLabelWidth)
        ) {
            const fontSize = this.config.baseFontSize ?? 10;
            const x = state.margins.left;
            const width = node.attrs.envelopeLabelWidth;
            const runs = node.children
                .slice(1)
                .flatMap((child) => this.buildInlineRuns(child));
            if (runs[0]) runs[0].text = runs[0].text.trimStart();
        ...
        if (
            ...
            const fieldType =
                /** @type {"text" | "signature" | undefined} */ (
                    ...
                    ? "signature"
                    : "text";
            const fieldName = /** @type {string | undefined} */ (
                node.attrs?.name ?? /** @type {any} */ (node).name
            );
            const fieldNameKey = /** @type {string | undefined} */ (
                node.attrs?.fieldNameKey ??
                    node.attrs?.field_name_key ??
                    /** @type {any} */ (node).fieldNameKey ??
                    /** @type {any} */ (node).field_name_key
            );
            const tooltip = /** @type {string | undefined} */ (
                node.attrs?.tooltip ?? /** @type {any} */ (node).tooltip
            );
            const value = /** @type {string | undefined} */ (
                node.attrs?.value ?? /** @type {any} */ (node).value
            );
            const readOnly =
                typeof (
                    node.attrs?.readOnly ?? /** @type {any} */ (node).readOnly
                ) === "boolean"
                    ? /** @type {boolean} */ (
                          node.attrs?.readOnly ??
                              /** @type {any} */ (node).readOnly
                      )
                    : undefined;
            const required =
                typeof (
                    node.attrs?.required ?? /** @type {any} */ (node).required
                ) === "boolean"
                    ? /** @type {boolean} */ (
                          node.attrs?.required ??
                              /** @type {any} */ (node).required
                      )
                    : undefined;
            const maxLengthRaw =
                node.attrs?.maxLength ??
                node.attrs?.max_length ??
                /** @type {any} */ (node).maxLength;
            const maxLength =
    ...
    splitInlineRunForPlaceholders(run) {
        if (
            this.config.pageComposition?.placeholderMode === "literal" &&
            (this.config.pageComposition.mode === "fixed" ||
                this._renderingPageComposition)
        ) {
            return [
                { kind: "text", ...run, text: run.formFieldRawText ?? run.text }
            ];
        }
        if (run.formField) {
            return [
    ...
    renderInlineRunsWrapped(
        ...
        for (let s = 0, sLen = segments.length; s < sLen; s++) {
            ...
            if (segment.kind === "placeholder") {
                ...
                const isTrailingOnLine =
                    !this.hasVisibleInlineContentBeforeNextLine(
                        segments,
                        s + 1
                    );
                ...
                builder.saveState();
                this._applyStrokeColor(builder, segment.color ?? 0);
                builder
                    .setLineWidth(thickness)
        ...
        // Trailing line spacing cannot create a page without following content.
    ...
    /**
     * Measure the initial header and configured minimum body rows that fit a page.
     * @param {TableNode} node
     * @param {PdfBuildState} state
     * @returns {number}
     */
    measureTableOpeningHeight(node, state) {
        const configuration = this.resolveTableRenderConfig(node);
        const baseFontSize = this.config.baseFontSize ?? 10;
        const columnCount = this.getTableColumnCount(node);
        const width = this._resolveTableBox(
            state,
            configuration,
            columnCount
        ).width;
        const fontSize = this._getTableFontSize(
            configuration,
            baseFontSize,
            width / Math.max(1, columnCount)
        );
        const defaultPadding = Math.max(2, 4 * (fontSize / baseFontSize));
        const bodyPadding = this._normalizeBoxPadding(
            configuration.cellPadding,
            defaultPadding
        );
        const headerPadding = hasProperty(configuration.header, "cellPadding")
            ? this._normalizeBoxPadding(
                  configuration.header.cellPadding,
                  defaultPadding
              )
            : bodyPadding;
        const borderWidth = Math.max(
            0,
            numberOr(configuration.borderWidth, 0.5)
        );
        const widths = this.calculateColumnWidths(node, width, columnCount, {
            state,
            fontSize,
            bodyPadding,
            headerPadding,
        ...
        const lineHeight = this._getTableLineHeight(
            configuration,
            this.config.lineHeight ?? 1.5
        );
        const hasHeader = node.attrs?.headerRow !== false;
        const minimumBodyRows = configuration.minimumBodyRows ?? 1;
        if (!Number.isInteger(minimumBodyRows) || minimumBodyRows < 1) {
            throw new Error(
                "Table minimumBodyRows must be a positive integer."
            );
        }
        let height = 0;
        const rowCount = Math.min(
            node.children.length,
            minimumBodyRows + (hasHeader ? 1 : 0)
        );
        for (let index = 0; index < rowCount; index++) {
            ...
            const rowHeight = this.calculateRowHeight(
                ...
                widths,
                fontSize,
                isHeader ? headerPadding : bodyPadding,
                isHeader,
                lineHeight,
                borderWidth
            );
            // Preserve at least the header and first body row; relax larger groups only when a full page cannot hold them.
            if (
                index > (hasHeader ? 1 : 0) &&
                height + rowHeight > state.contentHeight
            )
                break;
            height += rowHeight;
        }
    ...
    renderTable(node, state, sectionId, layout) {
        ...
        const avgColWidth = tableWidth / Math.max(1, columnCount);
        ...
        const borderWidth = Math.max(0, numberOr(tableCfg.borderWidth, 0.5));
        ...
        const outerBorderColor = tableCfg.outerBorderColor ?? borderColor;
        const headerBorderColor = tableCfg.header?.borderColor ?? borderColor;
        const uppercaseBaselineShift = tableCfg.uppercaseBaselineShift ?? 0.1;
        ...
        let fragmentTopY = tableY;
        const frameFragment = () => {
            if (fragmentTopY > tableY && borderWidth > 0) {
                this._drawStrokedRectColor(
                    state.currentPage.contentBuilder,
                    tableX,
                    tableY,
        ...
        for (
            ...
            // Reserve the configured body rows with the initial header.
            const requiredHeight =
                rowIdx === 0
                    ? this.measureTableOpeningHeight(node, state)
                    : rowHeight;
            if (tableY - requiredHeight < state.margins.bottom) {
                frameFragment();
                // Need new page
                this.newPage(state, sectionId);
            ...
            if (tableY - rowHeight < state.margins.bottom)
                throw new Error(
                    "A table row must fit on one page with its repeated header."
                );
            ...
            this.renderTableRow(
                ...
                rowHeight,
                borderWidth,
                isHeader ? headerBorderColor : borderColor,
                isHeader,
                tableFontSize,
                tableLineHeight,
                rowBg,
    ...
    /**
     * Measure the widest unwrapped cell with its header or body font, padding and border.
     * @param {TableNode} table
     * @param {number} columnIndex
     * @param {TableColumnMeasurement} measurement
     * @returns {number}
     */
    measureTableColumnWidth(table, columnIndex, measurement) {
        let contentWidth = 0;
        for (let rowIndex = 0; rowIndex < table.children.length; rowIndex++) {
            ...
            const isHeader =
                row.type === "header-row" ||
                (rowIndex === 0 && table.attrs?.headerRow !== false);
            const padding = isHeader
                ? measurement.headerPadding
                : measurement.bodyPadding;
            const font = this.getSemanticStyledFont(
                measurement.state,
                isHeader ? "table_header" : "table_body",
                this.getFont(measurement.state, isHeader),
                { bold: isHeader }
            );
            ...
            const textWidth = Math.max(
                ...
                ...text
                    .split(/\r?\n/)
                    .map((line) =>
                        measureTextWidth(line, font, measurement.fontSize)
                    )
            );
        ...
        // Preserve a single line through floating-point differences in cell placement.
        return Math.ceil(contentWidth * 1000 + 1) / 1000;
    }
     ...
     * @returns {number[]}
     */
    calculateColumnWidths(table, tableWidth, columnCount, measurement) {
        ...
        if (columns && columns.length > 0) {
            ...
            for (let i = 0; i < effectiveCount; i++) {
                ...
                if (
                    col.widthType === "max-content" ||
                    col.widthType === "max-content-padded"
                ) {
                    if (!measurement) {
                        throw new Error(
                            "Content-sized columns require table font and padding measurements."
                        );
                    }
                    const measuredWidth = this.measureTableColumnWidth(
                        table,
                        i,
                        measurement
                    );
                    widths.push(measuredWidth);
                    totalFixed += measuredWidth;
                    if (col.widthType === "max-content-padded")
                        paddedColumns.push(i);
                } else if (col.widthType === "fixed" && col.width) {
            ...
            if (paddedColumns.length > 0 && measurement) {
                let autoContentWidth = minAutoWidth;
                for (let index = 0; index < widths.length; index++) {
                    if (widths[index] === 0) {
                        autoContentWidth = Math.max(
                            autoContentWidth,
                            this.measureTableColumnWidth(
                                table,
                                index,
                                measurement
                            )
                ...
                // Auto columns share equal widths; reserve their widest unwrapped content before adding optional space.
                const availablePadding = Math.max(
                    0,
                    tableWidth - totalFixed - autoContentWidth * autoCount
                );
                const extraWidth = Math.min(
                    measurement.fontSize * 2,
                    availablePadding / paddedColumns.length
                );
                for (const index of paddedColumns) widths[index] += extraWidth;
                totalFixed += extraWidth * paddedColumns.length;
            }
     ...
     * @param {number} [uppercaseBaselineShift=0.1]
     * @param {boolean} [drawTopBorder=true] - Internal row boundary; the table owns the outside frame.
     * @returns {void}
     */
    renderTableRow(
        row,
        ...
        const rowText = row.children
            .map((cell) => this.extractText(cell))
            .join(" ");
        const opticalShift =
            /\p{Lu}/u.test(rowText) && rowText === rowText.toUpperCase()
                ? fontSize * uppercaseBaselineShift
                : 0;
        ...
        for (let i = 0; i < cellCount; i++) {
            ...
            if (i > 0 && borderWidth > 0 && !this._isColorNone(borderColor)) {
                builder.saveState().setLineWidth(borderWidth);
                this._applyStrokeColor(builder, borderColor);
                builder
                    .moveTo(cellX, y)
            ...
            const align = cell
                ? this.resolveNodeHorizontalAlign(cell, "left")
                : "left";
            ...
            const centerInk = cellVAlign === "middle";
            const firstInk = centerInk
                ? this.measureTextInk(
                      state,
                      isHeader
                          ? row.children
            ...
            const totalTextHeight = centerInk
            ...
            // Split body leading above and below each line's em box.
            const halfLeading = isHeader
                ? 0
                : (Math.max(0, lineHeight - 1) * fontSize) / 2;
            let textY = centerInk
                ? textBlockTopY - firstInk.ascent
                : textBlockTopY - fontSize * 0.8 - halfLeading - opticalShift;
        ...
        if (
            drawTopBorder &&
            borderWidth > 0 &&
            !this._isColorNone(borderColor)
        ) {
            builder.saveState().setLineWidth(borderWidth);
            this._applyStrokeColor(builder, borderColor);
            builder.moveTo(x, y).lineTo(cellX, y).stroke().restoreState();
        }
    }
    ...
    _drawStrokedRectColor(builder, x, y, width, height, lineWidth, color) {
        if (lineWidth <= 0 || this._isColorNone(color)) return;
        builder.saveState();
    ...
    /**
     * Render a loaded Markdown resource or an image node with a local path.
     * @param {Pick<BaseNode, "id" | "attrs">} node
     * @param {PdfBuildState} state
     * @returns {void}
     */
    renderImage(node, state) {
        const fontSize = this.config.baseFontSize ?? 10;
        const sourceBytes = /** @type {ImageData} */ (node.attrs).sourceBytes;
        ...
        if (
            sourceBytes &&
            !fullPage &&
            state.currentY - requestedHeight - fontSize < state.margins.bottom
        ) {
            this.newPage(state, state.currentPage.sectionId);
        }
        const page = /** @type {PdfPage} */ (state.currentPage);
        const builder = page.contentBuilder;

        if (!sourceBytes && (!imagePath || !existsSync(imagePath))) {
            const x =
        ...
        const imageBytes = sourceBytes ?? readFileSync(imagePath);
        const artwork =
            extname(imagePath ?? "").toLowerCase() === ".svg"
                ? processDustCover(new TextDecoder().decode(imageBytes), {
                      cacheDir: null
                  })
                : null;
        if (
            artwork &&
            (artwork.root.children.length !== 1 ||
                artwork.root.children[0].tag !== "svg" ||
                artwork.root.text.trim())
        ) {
            throw new Error("An SVG image requires an SVG root element.");
        }
        if (
            artwork &&
            (![artwork.svgWidth, artwork.svgHeight].every(Number.isFinite) ||
                artwork.svgWidth <= 0 ||
                artwork.svgHeight <= 0)
        ) {
            throw new Error(
                "An SVG image requires positive intrinsic dimensions."
            );
        }
        const imageName = `IMG_${String(
            node.id ?? basename(imagePath ?? "image", extname(imagePath ?? ""))
        )}`
            .replace(/[^A-Za-z0-9_]+/g, "_")
            .replace(/_+/g, "_")
            .replace(/^_+|_+$/g, "");
        const xObjectName = artwork
            ? null
            : state.doc.embedImageXObject(imageName, imageBytes);
        const embeddedImageInfo = xObjectName
            ? state.doc.images.get(xObjectName)
            : null;
        const intrinsicWidth =
            artwork?.svgWidth ??
            embeddedImageInfo?.parsed?.width ??
            requestedWidth;
        const intrinsicHeight =
            artwork?.svgHeight ??
            embeddedImageInfo?.parsed?.height ??
            requestedHeight;
        ...
        if (artwork) {
            renderSvgToPdf(artwork, builder, state, {
                x: drawX,
                top: state.pageHeight - drawY - drawHeight,
                width: drawWidth,
                height: drawHeight
            });
        } else {
    ...
    /**
     * The block node types a list item may hold beneath its own line. A nested list is the common
     * one: a tight list's item holds its text nodes and then the nested list as siblings, with no
     * paragraph between them. Anything here is rendered as a block under the item's text indent;
     * anything else is part of the item's inline run.
     * @type {ReadonlySet<string>}
     */
    static LIST_ITEM_BLOCK_TYPES = new Set([
        "list",
        "paragraph",
        "definition",
        "table",
    ...
    /**
     * Split a list item into the node its inline run is built from and the children rendered as
     * blocks after it.
     *
     * A loose item (a paragraph first) uses that paragraph as the run; a tight item's run is its
     * own inline children up to the first block child. In both cases every block child from that
     * point on is rendered beneath the run rather than folded into it.
     *
     * @param {BaseNode} node
     * @returns {{ inlineRoot: BaseNode | null; inlineChildren: BaseNode[]; blockStart: number }}
     */
    splitListItem(node) {
        ...
        for (let i = 0, len = children.length; i < len; i++) {
            if (
                TwoPassPdfRenderer.LIST_ITEM_BLOCK_TYPES.has(children[i].type)
            ) {
                firstBlock = i;
                break;
            }
        ...
        if (firstBlock < 0) {
            return {
                inlineRoot: node,
                inlineChildren: [],
                blockStart: children.length
            };
        }
        const first = children[firstBlock];
        if (
            firstBlock === 0 &&
            (first.type === "paragraph" || first.type === "definition")
        ) {
            return { inlineRoot: first, inlineChildren: [], blockStart: 1 };
        }
        return {
            inlineRoot: null,
            inlineChildren: children.slice(0, firstBlock),
            blockStart: firstBlock
        };
    }

    /**
     * The inline runs of a list item's own line.
     * @param {{ inlineRoot: BaseNode | null; inlineChildren: BaseNode[] }} split
     * @returns {ReturnType<TwoPassPdfRenderer["buildInlineRuns"]>}
     */
    listItemInlineRuns(split) {
        if (split.inlineRoot !== null)
            return this.buildInlineRuns(split.inlineRoot);
        /** @type {ReturnType<TwoPassPdfRenderer["buildInlineRuns"]>} */
        const runs = [];
        for (let i = 0, len = split.inlineChildren.length; i < len; i++) {
            this.collectInlineRunsInto(runs, split.inlineChildren[i], {
                bold: false,
                italic: false,
                underline: false,
                monospace: false
    ...
    renderListItem(node, state, sectionId, layout, depth) {
        ...
        const split = this.splitListItem(node);
        const runs = this.listItemInlineRuns(split);
        if (runs.length > 0) {
            state.currentY = this.renderInlineRunsWrapped(
                runs,
                state,
        ...
        state.lastNodeType =
            split.inlineRoot === null || split.inlineRoot === node
                ? "list-item"
                : split.inlineRoot.type;

        // Render the block children (nested lists, further paragraphs, tables) under the bullet's
        // text indent, not under the marker. The indent is carried by the margin, so the nesting
        // depth starts again from it rather than compounding.
        if (split.blockStart < node.children.length) {
            const originalLeft = state.margins.left;
            const originalWidth = state.contentWidth;
            ...
            for (
                let i = split.blockStart, len = node.children.length;
                i < len;
                i++
            ) {
    ...
    renderSignatureBlock(node, state) {
        if (node.attrs.variant === "split")
            return this._renderSplitSignatureBlock(node, state);
        if (node.attrs.variant === "single")
            return this._renderSingleSignatureBlock(node, state);
        ...
        const signatureRowHeight = numberOr(
            ...
            Math.max(64, fontSize * lineHeight * 2)
        );
        ...
        if (spec.partyLabel) {
            const bandBottomY = state.currentY - titleFontSize - headerGap;
            const bandHeight = boxTopY - bandBottomY;
            if (
            ...
            const titleMetrics = state.doc.embeddedFonts.get(
                this.getFontResourceName(state, boldFont)
            )?.metrics;
            const titleAscent = titleMetrics
                ? titleMetrics.capHeight / titleMetrics.unitsPerEm
                : 0.7;
            const titleDescent = this._textHasDeepDescenders(spec.partyLabel)
                ? titleMetrics
                    ? -titleMetrics.descender / titleMetrics.unitsPerEm
                    : 0.2
                : 0;
            const titleBaseline =
                bandBottomY +
                bandHeight / 2 -
                (titleFontSize * (titleAscent - titleDescent)) / 2;
            this.renderTextAt(
                startBuilder,
                state,
                spec.partyLabel,
    ...
    /**
     * Open signature layouts share one measurement path with pagination.
     * @param {BaseNode} node
     * @param {PdfBuildState} state
     * @param {number} [contentWidth]
     */
    _measureSingleSignatureBlock(
        node,
        state,
        contentWidth = state.contentWidth
    ) {
        const spec = this._getSignatureBlockSpec(node);
        const style = this._resolveDirectiveStyle("signature-block", "single");
        const fontSize = numberOr(
            style.fontSize,
            this.config.baseFontSize ?? 10
        );
        const titleGap = numberOr(style.headerGap, 5);
        const signatureHeight = numberOr(style.signatureRowHeight, 64);
        const rowHeight = numberOr(style.rowHeight, fontSize * 1.5);
        const rowGap = numberOr(style.rowGap, 0);
        const postGap = numberOr(style.postGap, 6);
        const font = this.getFont(state, false);
        const title = spec.partyLabel
            ? this.wrapText(spec.partyLabel, contentWidth, font, fontSize)
            : [];
        const body = spec.bodyText
            ? this.wrapText(spec.bodyText, contentWidth, font, fontSize)
            : [];
        const labeledFields = spec.fields.filter(
            (label) =>
                !this._isSignatureLineField(label) &&
                !(
                    /^(name|name and capacity)$/i.test(label) &&
                    spec.values[label]?.trim()
                )
        );
        const labelWidth = labeledFields.length
            ? Math.max(
                  ...labeledFields.map((label) =>
                      measureTextWidth(label + ":", font, fontSize)
                  )
              ) +
              fontSize * 0.75
            : 0;
        if (labelWidth > contentWidth - 48)
            throw new Error(
                "Single signature-block labels leave insufficient field width"
            );
        const height =
            title.length * fontSize * 1.3 +
            (title.length ? titleGap : 0) +
            body.length * fontSize * 1.3 +
            (body.length ? titleGap : 0) +
            spec.fields.reduce(
                (sum, field) =>
                    sum +
                    (this._isSignatureLineField(field)
                        ? signatureHeight
                        : rowHeight),
                0
            ) +
            Math.max(0, spec.fields.length - 1) * rowGap +
            postGap;
        return {
            spec,
            style,
     ...
     * @returns {void}
     */
    _renderSingleSignatureBlock(node, state, left, contentWidth) {
        const m = this._measureSingleSignatureBlock(node, state, contentWidth);
        if (state.currentY - m.height < state.margins.bottom)
            this.newPage(state, state.currentPage.sectionId);
        if (state.currentY - m.height < state.margins.bottom)
            throw new Error(
                "Single signature-block exceeds the available page height"
            );
        const builder = state.currentPage.contentBuilder;
        const x = left ?? state.margins.left;
        const width = contentWidth ?? state.contentWidth;
        for (const lines of [m.title, m.body]) {
            for (const line of lines) {
                state.currentY -= m.fontSize;
                this.renderTextAt(
                    builder,
                    state,
                    line,
                    ...
                    m.font,
                    "left",
                    m.style.titleColor ?? "#61727E"
                );
                state.currentY -= m.fontSize * 0.3;
            }
            if (lines.length) state.currentY -= m.titleGap;
        }
        for (let i = 0; i < m.spec.fields.length; i++) {
            const label = m.spec.fields[i];
            const signature = this._isSignatureLineField(label);
            const height = signature ? m.signatureHeight : m.rowHeight;
            const top = state.currentY;
            const bottom = top - height;
            const value = m.spec.values[label]?.trim();
            const identity = /^(name|name and capacity)$/i.test(label);
            const labelText =
                signature || (identity && value) ? "" : label + ":";
            const labelWidth = labelText ? m.labelWidth : 0;
            if (signature)
                this.drawLine(
                    builder,
                    x,
                    bottom,
                    x + width,
                    numberOr(m.style.borderWidth, 0.5),
                    m.style.borderColor ?? "#213C51"
                );
            else if (labelText)
                this.renderTextAt(
                    builder,
                    ...
                    labelText,
                    x,
                    bottom + formFieldTextBaseline(height, m.fontSize),
                    m.fontSize,
                    m.font,
                    "left",
                    m.style.valueColor ?? "#263642"
                );
            const key = label
                .toLowerCase()
                .replace(/[^a-z0-9]+/g, "_")
                .replace(/_+$/g, "");
            state.formFieldCounter++;
            this.addFormFieldAnnotation(state, {
                type: "form",
                fieldType: signature ? "signature" : "text",
                name: node.attrs.fieldName
                    ? signature
                        ? node.attrs.fieldName
                        : node.attrs.fieldName + "." + key
                    : "signature_block." + state.formFieldCounter + "." + key,
                tooltip:
                    (m.spec.partyLabel ? m.spec.partyLabel + " — " : "") +
                    label,
                // Text appearances inset their ink by two points inside the widget.
                x: x + labelWidth - (signature ? 0 : 2),
                y: bottom + (signature ? 1 : 0),
                width: width - labelWidth + (signature ? 0 : 2),
                height: height - (signature ? 2 : 0),
                value: signature ? undefined : value || undefined,
                readOnly:
                    !signature && Boolean(value) && !/^\[[^\]]+\]$/.test(value),
                fontSize: m.fontSize,
                fontName: this.getFont(state, identity)
            });
            state.currentY =
                bottom - (i + 1 < m.spec.fields.length ? m.rowGap : 0);
        }
        this._recordInkBottomY(state, state.currentY);
        state.currentY -= m.postGap;
    }

    /** @param {BaseNode} node @param {PdfBuildState} state @returns {void} */
    _renderSplitSignatureBlock(node, state) {
        if (
            node.children.length !== 2 ||
            node.children.some((child) => child.attrs.variant !== "single")
        )
            throw new Error("Split signature-block requires two single blocks");
        const style = this._resolveDirectiveStyle("signature-block", "split");
        const gap = numberOr(style.columnGap, 40);
        const width = state.contentWidth;
        if (gap < 0 || (width - gap) / 2 < 120)
            throw new Error("Split signature-block columns are too narrow");
        const height = this._estimateSignatureBlockHeight(node, state);
        if (state.currentY - height < state.margins.bottom)
            this.newPage(state, state.currentPage.sectionId);
        if (state.currentY - height < state.margins.bottom)
            throw new Error(
                "Split signature-block exceeds the available page height"
            );
        const top = state.currentY;
        const left = state.margins.left;
        ...
        for (let i = 0; i < 2; i++) {
            state.currentY = top;
            this._renderSingleSignatureBlock(
                node.children[i],
                state,
        ...
        this.drawFilledRect(
            ...
            left + width / 2,
            top - height,
            numberOr(style.separatorWidth, 0.6),
            height,
            style.separatorColor ?? "#D3DCE2"
        );
        state.currentY = top - height;
        this._recordInkBottomY(state, state.currentY);
    }
     ...
     * @returns {number}
     */
    _estimateSignatureBlockHeight(
        node,
        state,
        contentWidth = state.contentWidth
    ) {
        if (node.attrs.variant === "single")
            return this._measureSingleSignatureBlock(node, state, contentWidth)
                .height;
        if (node.attrs.variant === "split") {
            const style = this._resolveDirectiveStyle(
                "signature-block",
            ...
            const columnWidth =
                (contentWidth - numberOr(style.columnGap, 40)) / 2;
            return Math.max(
                ...node.children.map((child) =>
                    this._estimateSignatureBlockHeight(
                        child,
                        state,
                        columnWidth
                    )
        ...
        let height = pad.top + pad.bottom + fontSize * 0.2;
        if (spec.partyLabel) {
        ...
        if (spec.bodyText) {
            const wrappedBody = layoutPlainText(
                spec.bodyText,
                Math.max(120, contentWidth - pad.left - pad.right),
                this.getFont(state, false),
                fontSize,
                fontSize * lineHeight
            );
    ...
    addHeadersFooters(state, sections, layout) {
        ...
        // Count numbered pages using the configured cover and signing-page exclusions.
    ...
    estimateNodeHeight(node, state, depth = 0) {
        ...
        switch (node.type) {
            case "paragraph": {
                if (isNumber(node.attrs.envelopeLabelWidth)) {
                    const runs = node.children
                        .slice(1)
                        .flatMap((child) => this.buildInlineRuns(child));
                    return (
                        this.estimateInlineRunsLineCount(
                            runs,
                            state,
                            state.contentWidth - node.attrs.envelopeLabelWidth,
                            baseFontSize
                        ) *
                        baseFontSize *
                        lineHeight
            ...
            case "list-item": {
                ...
                let height = lines * baseFontSize * lineHeight;
                if (split.blockStart < node.children.length) {
                    // The blocks render under the text indent, so their width is what is left of it.
                    const originalWidth = state.contentWidth;
                    state.contentWidth = Math.max(1, availableWidth);
                    for (
                        let i = split.blockStart, len = node.children.length;
                        i < len;
                        i++
            ...
            case "form-field":
                return this.measureTextArea(node, state).height;
            ...
            case "section": {
                if (node.attrs.directive === "form-fields")
                    return this.measureFormFields(node, state).height;
                let contentHeight = 0;
                for (let i = 0, len = node.children.length; i < len; i++) {
                    contentHeight += this.estimateNodeHeight(
                        node.children[i],
    ...
    buildPdf(state) {
        ...
        for (let i = 0, len = state.pages.length; i < len; i++) {
            ...
            const pagination = this.config.pageComposition?.pagination;
            if (page.ownsPageFurniture && pagination) {
                const total = state.pages.filter(
                    (candidate) => candidate.pageNumber > 0
                ).length;
                const text = "Page " + page.pageNumber + " of " + total;
                const font = this.getFont(state, false);
                if (
                    measureTextWidth(text, font, pagination.fontSize) >
                    pagination.width
                )
                    throw new Error(
                        "Page composition pagination exceeds its width"
                    );
                this.renderTextAt(
                    page.headerFooterBuilder,
                    state,
                    text,
                    pagination.x + pagination.width,
                    state.pageHeight - pagination.top - pagination.fontSize,
                    pagination.fontSize,
                    font,
                    "right",
                    pagination.color
            ...
            const trackingConfig = this.config.pageTracking;
            const suppressTracking =
                ((page.sectionId === "cover" ||
                    page.sectionId === "dust-cover") &&
                    trackingConfig?.suppressCover === true) ||
                (page.sectionId === "signing" &&
                    trackingConfig?.suppressSigning !== false);
            if (trackingConfig?.enabled && !suppressTracking) {
                const tracking = layoutPageTracking(this.config.pageTracking, {
                    pageWidth: state.pageWidth,
                    pageHeight: state.pageHeight,
                    rightMargin: state.baseMargins.right,
                    pageNumber: i + 1,
                    totalPages: len,
                    documentId: String(this.config.variables?.documentId ?? "")
                });
                if (tracking) {
                    combinedStream +=
                        "\n" +
                        renderPageTracking(
                            tracking,
                            state.doc.registerFont("Helvetica")
                        );
                }
            }
