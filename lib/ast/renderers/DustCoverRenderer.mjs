...
import { SvgParser } from "../../parsing/SvgParser.mjs";
import {
    GoogleFontFetcher,
    normaliseFontFamily,
    parseFontWeight,
...
/**
 * @typedef {import("../../parsing/SvgParser.mjs").SvgElement} SvgElement
...
/**
 * @typedef {Object} SvgViewport
 * @property {number} x - PDF points from the physical page's left edge.
 * @property {number} top - PDF points from the physical page's top edge.
...
export function processDustCover(svgContent, options) {
    ...
    const root = SvgParser.parse(preparedSvgContent);
    const {
        minX: svgMinX,
        minY: svgMinY,
        width: svgWidth,
 ...
 * @param {SvgViewport} [viewport] - Omit to use the entire physical page.
 * @returns {void}
 */

export function renderSvgToPdf(dustCoverPage, builder, state, viewport) {
    const {
        root,
        svgMinX = 0,
        svgMinY = 0,
    ...
    const pageWidth = viewport?.width ?? state.pageWidth;
    const pageHeight = viewport?.height ?? state.pageHeight;
    const offsetX = viewport?.x ?? 0;
    const offsetY = viewport
        ? state.pageHeight - viewport.top - viewport.height
        : 0;
    if (
        viewport &&
        (![viewport.x, viewport.top, viewport.width, viewport.height].every(
            Number.isFinite
        ) ||
            viewport.width <= 0 ||
            viewport.height <= 0)
    ) {
        throw new Error("Invalid SVG viewport");
    }
    ...
    if (fonts) {
        v(
            `fonts size=${fonts.size} families=[${Array.from(fonts.keys()).join(
                ", "
            )}]`
        );
        fonts.forEach((info, family) => {
            v(
                `  "${family}" format=${info.format} bytes=${
                    info.bytes?.length ?? 0
                }`
            );
        });
    ...
    const px = (sx) => offsetX + (sx - svgMinX) * scaleX;
    const py = (sy) => offsetY + pageHeight - (sy - svgMinY) * scaleY;
    ...
    function renderEl(el, gTx, gTy, gScaleX, gScaleY, inheritedStyle) {
        ...
        switch (el.tag) {
            ...
            case "path": {
                const segments = SvgParser.parseSvgPath(el.attrs["d"] ?? "");
                if (segments.length === 0) break;
                const fill = resolveColor(resolvedStyle["fill"] ?? "#000000");
                const stroke = resolveColor(resolvedStyle["stroke"] ?? null);
                if (!fill && !stroke) break;
                const local = parseTransform(el.attrs["transform"]);
                const pathX = (value) =>
                    px((value * local.scaleX + local.tx) * gScaleX + gTx);
                const pathY = (value) =>
                    py((value * local.scaleY + local.ty) * gScaleY + gTy);
                builder.saveState();
                    ...
                    builder.setLineWidth(
                        psl(
                            _parseSvgLength(resolvedStyle["stroke-width"], 1) *
                                Math.max(
                                    Math.abs(gScaleX * local.scaleX),
                                    Math.abs(gScaleY * local.scaleY)
                                )
                        )
                    );
                    builder.setLineJoin(
                        resolvedStyle["stroke-linejoin"] === "round"
                            ? 1
                            : resolvedStyle["stroke-linejoin"] === "bevel"
                            ? 2
                            : 0
                    );
                    builder.setLineCap(
                        resolvedStyle["stroke-linecap"] === "round"
                            ? 1
                            : resolvedStyle["stroke-linecap"] === "square"
                            ? 2
                            : 0
                    );
                }
                for (const segment of segments) {
                    if (segment.command === "M")
                        builder.moveTo(pathX(segment.x), pathY(segment.y));
                    else if (segment.command === "L")
                        builder.lineTo(pathX(segment.x), pathY(segment.y));
                    ...
                    else builder.closePath();
                }
                const evenOdd = resolvedStyle["fill-rule"] === "evenodd";
                if (fill && stroke) {
                    if (evenOdd) builder.fillEvenOddAndStroke();
                    else builder.fillAndStroke();
                } else if (fill) {
                    if (evenOdd) builder.fillEvenOdd();
                    else builder.fill();
                } else builder.stroke();
