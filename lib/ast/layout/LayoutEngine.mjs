...
const DEFAULT_HEIGHT_CALCULATORS = {
    ...
    // =========================================================================
    image: (node, ctx) => {
        const height = /** @type {number} */ (node.attrs?.height) ?? 200;
        const captionHeight = node.attrs?.alt
            ? ctx.baseFontSize * ctx.lineHeight
            : 0;
        const totalHeight = height + captionHeight + 10; // Spacing
        return {
            height: totalHeight,
            canSplit: false,
...
export class LayoutEngine {
    ...
    buildContext(pageConfig, sectionId, headers, footers) {
        ...
        const width = pageConfig.width ?? dims.width;
        const height = pageConfig.height ?? dims.height;
