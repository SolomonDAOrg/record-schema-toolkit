...
import { BaseNode, TextNode, BreakNode, FormFieldNode } from "../nodes/BaseNode.mjs";
...
export class MarkdownToAstConverter {
    ...
    _convertSignatureBlockDirective(mdNode) {
        ...
        return new SignatureBlockNode({
            ...
            partyLabel: title.length > 0 ? title : undefined,
    ...
    _convertFormField(mdNode) {
        const attrs = mdNode.attrs && typeof mdNode.attrs === "object"
            ? mdNode.attrs
            : {};

        const placeholderText =
            typeof attrs.placeholderText === "string" && attrs.placeholderText.trim().length > 0
                ? attrs.placeholderText.trim()
                : mdNode.content.trim();

        const node = new FormFieldNode(placeholderText, {
            fieldType:
                attrs.fieldType === "signature" ? "signature" : "text",
            rawText:
                typeof attrs.rawText === "string" && attrs.rawText.length > 0
                    ? attrs.rawText
                    : `[${placeholderText}]`,
            fieldNameKey:
                typeof attrs.fieldNameKey === "string" && attrs.fieldNameKey.length > 0
                    ? attrs.fieldNameKey
                    : undefined,
            name:
            ...
            readOnly:
                typeof attrs.readOnly === "boolean" ? attrs.readOnly : undefined,
            required:
                typeof attrs.required === "boolean" ? attrs.required : undefined,
            maxLength:
                typeof attrs.maxLength === "number" && Number.isFinite(attrs.maxLength)
                    ? attrs.maxLength
                    : undefined
        });
