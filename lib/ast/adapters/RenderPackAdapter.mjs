 ...
 *
 * Converts render-pack configuration at the AST boundary.
 * record-schema library speaks snake_case; AST library speaks camelCase.
 *
 * @module format-ast/adapters/RenderPackAdapter
 */

import { RenderPack } from "../../record-schema/RenderPack.mjs";
import { isArray } from "../../util/general.mjs";
import { deepMerge, isObject } from "../../util/objects.mjs";
 ...
 * @typedef {import("../types/core.mjs").RenderPackData} LegacyRenderPackData
 * @typedef {import("../../record-schema/types/general.mjs").RulesetSpacingPolicy} SpacingPolicy
 * @typedef {import("../../record-schema/types/general.mjs").RenderPackData} RenderPackData
 * @typedef {import("../../record-schema/types/general.mjs").RenderDocumentPolicy} RenderDocumentPolicy
 * @typedef {import("../../record-schema/types/general.mjs").RenderTarget} RenderTarget
 * @typedef {import("../../record-schema/types/general.mjs").RenderProfile} RenderProfile
 * @typedef {import("../../record-schema/types/general.mjs").RulesetRender} ResolvedRenderConfig
 * @typedef {import("../../record-schema/types/general.mjs").RulesetMargins} MarginsConfig
 * @typedef {import("../../record-schema/types/general.mjs").RulesetHorizontalRule} HorizontalRuleConfig
 * @typedef {import("../../record-schema/types/general.mjs").RulesetFonts} FontsConfig
 * @typedef {import("../../record-schema/types/general.mjs").RenderRuleset} RenderRuleset
 */
...
export class RenderPackAdapter {
    /**
     * @param {RenderPackData | LegacyRenderPackData} pack
     */
    constructor(pack) {
        /** @type {RenderPackData} */
    ...
    resolveProfile(profile_id) {
        return (
            new RenderPack(this.pack).getEffectiveTargetConfig(profile_id)
                ?.target || null
        );
    }
    ...
    _mergeConfigs(base, overlay) {
        return /** @type {ResolvedRenderConfig} */ (deepMerge(base, overlay));
    }
     ...
     */
    getPacketConfig() {
        const pc = this.pack.packet_config;
    ...
    normalizeTocLevelStyles(level_styles) {
        ...
        for (const key of Object.keys(level_styles)) {
            ...
            const raw = level_styles[level];
            if (!isObject(raw)) {
                out[level] = raw;
                continue;
            }
            out[level] = /** @type {Metadata} */ (toCamelCase(raw));
        }
        return out;
    }
     ...
     *
     * @param {{ rel_path: string, doc_type: string | null, ext: string | null, is_root_file?: boolean, render_profile_id?: string }} file
     * @returns {ResolvedRenderConfig | null}
     */
    resolveForFile(file) {
        const rulesets = this.policy.rulesets;
        ...
        if (profile_id) {
            const configuration = this.resolveProfile(profile_id);
            if (!configuration) return null;
            return file.render_profile_id
                ? this._mergeConfigs(effective, configuration)
                : this._mergeConfigs(configuration, effective);
        }
        ...
        if (target_id) {
            const configuration = this.resolveTarget(target_id);
...
/**
 * @param {{ pageSize?: string, orientation?: string, margins?: import("../types/core.mjs").Margins }} configuration
 * @returns {PageConfig}
 */
export function resolvePageConfiguration(configuration) {
    /** @type {PageConfig} */
    const page = {};
    if (configuration.pageSize !== undefined) {
        const size = configuration.pageSize.toLowerCase();
        switch (size) {
            case "letter":
            case "legal":
            case "a4":
            ...
            default:
                throw new RangeError(
                    `Unsupported page size: ${configuration.pageSize}`
                );
        }
    }
    if (configuration.orientation !== undefined) {
        const orientation = configuration.orientation.toLowerCase();
        if (orientation !== "portrait" && orientation !== "landscape") {
            throw new RangeError(
                `Unsupported page orientation: ${configuration.orientation}`
            );
        }
        page.orientation = orientation;
    }
    if (configuration.margins !== undefined)
        page.margins = configuration.margins;
    return page;
}
